...
## 目标特性 (Target Feature)
展示 Rust 函数如何导出为 extern "C" 以及 ABI 约定降级。

## 降级逻辑预演 (Expected Lowering Logic)
1. **纯粹的前端责任**：SA-ASM 是一个无 AST、无全局类型推导的物理执行验证机。像 Extern C ABI 这样的高级语义或外部抽象，100% 由前端（如 Rustc, TypeScript Compiler 或大语言模型）在降级期间展开。
