# 144 - PhantomData Marker

## 目标特性 (Target Feature)
展示 PhantomData 幽灵数据在降级时物理体积为 0 且无状态追踪。

## 降级逻辑预演 (Expected Lowering Logic)
1. **纯粹的前端责任**：SA-ASM 是一个无 AST、无全局类型推导的物理执行验证机。像 PhantomData Marker 这样的高级语义或外部抽象，100% 由前端（如 Rustc, TypeScript Compiler 或大语言模型）在降级期间展开。
