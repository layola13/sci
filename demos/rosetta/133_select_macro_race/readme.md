...
## 目标特性 (Target Feature)
展示 tokio::select! 如何展平为多分支 poll 状态机。

## 降级逻辑预演 (Expected Lowering Logic)
1. **纯粹的前端责任**：SA-ASM 是一个无 AST、无全局类型推导的物理执行验证机。像 Select Macro Race 这样的高级语义或外部抽象，100% 由前端（如 Rustc, TypeScript Compiler 或大语言模型）在降级期间展开。
