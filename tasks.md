...
## Current entry-dir no_follow authorization and stale lock accounting batch (2026-07-17)

- [x] Keep the worktree scoped to cache path-authorization/lock accounting only.
- [x] Open project-cache entry directories with `no_follow` and reject symlink entry dirs as `incomplete`/`entry.symlink`.
- [x] Reclaim unowned stale `.locks` entry/build lock files during clean while active writers stay pinned.
- [x] Run focused unit filters, format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining full openat/O_NOFOLLOW path authorization, emit/sync/rename/link external-tool injection, cross-process recovery, and native macOS/Windows validation beyond this Linux unit batch.
