...
- [x] `std::num::NonZeroI*::isqrt` helper 批次已完成：新增 `NONZERO_I{8,16,32,64,SIZE}_ISQRT`，按 Rust `NonZero<signed>::isqrt` 语义对正数 NonZero wrapper 调用对应 signed primitive `isqrt` 并写回同一 NonZero 布局；负数沿用 primitive signed `isqrt` 的 SA `panic(2203)` 控制流。新增 focused 单测并同步 `progress/current_plan/std_missing`。不主张 Rust panic message/object identity、generic `NonZero<T>`、`i128` 或 trait-level 集成。
