...
- [x] `std::num::{Wrapping,Saturating}` operator 宏表面批次已完成：为现有 `WrappingU64` / `WrappingU32` / `WrappingI64` 布局新增 `*_DIV` / `REM` / `SHL` / `SHR` / `NOT` / `BITAND` / `BITOR` / `BITXOR` concrete helper，并为 `WrappingI64` 与 `SaturatingI64` 新增 `NEG` / `ABS` / `SIGNUM` / `IS_POSITIVE` / `IS_NEGATIVE` signed helper；`WrappingU32` 按 32-bit 宽度 mask。新增 focused 单测并同步 `progress/current_plan/std_missing`。暂不新增 Saturating div/rem，因为 Rust 对 zero divisor 保持 panic 语义，而现有 SA saturating division helper 是非 panic convenience。
