...
- [x] Add test-only `spawn`/`run` injection around `zig cc` process wrappers.
- [x] Route objcopy localization through the same `runProcess` wrapper.
- [x] Run focused driver unit tests (`6/6`), format/diff checks, then commit/push the selected driver/docs files.
- [ ] Broaden remaining end-to-end CLI emit/link failure injection, cross-process recovery, and native macOS/Windows validation beyond this Linux driver-unit batch.

## Current final-component no_follow cache file open batch (2026-07-17)

- [x] Keep the worktree scoped to project-cache final-component path authorization only.
- [x] Open cache hash/sync/copy/manifest/artifact payloads without following a final-component symlink (`openFileNoFollowFinal` / `O_NOFOLLOW`).
- [x] Reject symlink sources on cache copy and refuse symlink hash/sync IO.
- [x] Keep lookup/first-difference fail-closed when a regular file is replaced by a symlink.
- [x] Run focused project-cache unit filters (`31/31`), format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining full parent-chain openat/O_NOFOLLOW authorization, emit/link external-tool injection, cross-process recovery, and native macOS/Windows validation beyond this Linux unit batch.

## Current copy IO injection and incremental functions no_follow batch (2026-07-17)

- [x] Keep the worktree scoped to project-cache store IO/authorization only.
- [x] Inject low-level `copy` failures with `copy_artifact`/`copy_output` stage telemetry and no partial entries.
- [x] Reject symlink incremental `functions` directories with no_follow completeness/allowlist checks.
- [x] Run focused project-cache unit filters (`28/28`), format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining full openat/O_NOFOLLOW path authorization, emit/link external-tool injection, cross-process recovery, and native macOS/Windows validation beyond this Linux unit batch.

## Current sync/rename IO injection, entry no_follow, and stale lock reclaim batch (2026-07-17)

- [x] Keep the worktree scoped to project-cache store failure/authorization/lock work only.
- [x] Inject low-level `sync`/`rename` IO failures with `copy_artifact`/`manifest`/`publish` stage telemetry and no partial entries.
- [x] Open project-cache entry directories with `no_follow` and reject symlink entry dirs as `incomplete`/`entry.symlink`.
- [x] Reclaim unowned stale `.locks` entry/build lock files during clean without counting them in entry kept/removed stats.
- [x] Keep active staging writers pinned under exclusive locks.
- [x] Run focused project-cache unit filters (`27/27`), format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining full openat/O_NOFOLLOW path authorization, emit/link external-tool injection, cross-process recovery, and native macOS/Windows validation beyond this Linux unit batch.

## Current orphaned staging crash recovery batch (2026-07-17)

- [x] Keep the worktree scoped to staging crash-recovery only.
- [x] Recover same-key orphan staging directories under the exclusive store lock.
- [x] Keep active staging writers pinned while unpinned orphans are cleaned.
- [x] Run focused unit filters, format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining emit/sync/rename/link external-tool injection, cross-process crash recovery, TOCTOU-hard path authorization, and native macOS/Windows validation beyond this Linux unit batch.

## Current invalid-entry republish repair and manifest/publish failure injection batch (2026-07-17)

- [x] Keep the worktree scoped to cache store repair/injection only.
- [x] Repair unusable final project-cache entries on republish under the exclusive store lock.
- [x] Inject `manifest`/`publish` store failures without leaving partial entries.
- [x] Open entry validity/complete/clean paths with iterate permissions for allowlist checks.
- [x] Run focused unit filters, format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining emit/sync/rename/link external-tool injection, crash recovery, TOCTOU-hard path authorization, cross-process, and native macOS/Windows validation beyond this Linux unit batch.
...
- [x] Publish host link-policy identity into `build-exe`/`test` keys as `link_flags`, including explicit default export-dynamic policy.
...
- [x] Keep the worktree scoped to `src/cli.zig` corruption guards only.
- [x] Reject unexpected project-cache entry files outside the kind allowlist.
- [x] Reject oversize manifests before JSON parse as `manifest_invalid` / `manifest.size`.
- [x] Keep same-size output content flips as `artifact_corrupt` / `output.sha256`.
- [x] Run focused unit filters, format/diff checks, then commit/push the selected cache files.
- [ ] Broaden remaining failure-injection/TOCTOU/crash-recovery/cross-process and native macOS/Windows validation beyond this Linux unit batch.

## Current native evidence success-only upload hardening batch (2026-07-17)

- [x] Keep the worktree clean before starting the batch.
- [x] Change macOS native runtime/smoke evidence artifact uploads to `if: success()` so failed or rejected evidence is not uploaded.
- [x] Change Windows native runtime/smoke evidence artifact uploads to `if: success()` so failed or rejected evidence is not uploaded.
- [x] Lock the condition with Linux-runnable macOS/Windows source contracts that also reject `if: always()`.
- [x] Run focused workflow/CI contract validation, then commit/push the selected portability files.
- [ ] Execute the updated native workflows on macOS/Windows runners; this batch is workflow/source-contract evidence only until then.

## Current native evidence exact-schema hardening batch (2026-07-17)

- [x] Keep the worktree clean before starting the batch.
- [x] Reject unexpected top-level fields in native smoke/runtime evidence JSON.
- [x] Lock the exact-schema behavior with Linux-runnable validator tests and macOS/Windows source contracts.
...
- [x] Keep the worktree clean before starting the batch.
- [x] Make macOS native smoke derive `zig_version` from `zig version` and `llvm_version` from `LLVM_CONFIG --version` when workflow environment values are absent.
- [x] Make Windows native smoke derive `zig_version` from `zig version` and parse `llvm_version` from `clang --version` when workflow environment values are absent.
- [x] Lock the fallback behavior with Linux-runnable macOS/Windows source contracts.
- [x] Run focused syntax/format/contract validation, then commit/push the selected portability files.
- [ ] Execute the updated native workflows on macOS/Windows runners; this batch is script/source-contract evidence only until then.
...
- [ ] **[PARTIAL] P0.3 artifact key v4**: compiler/Zig and host target identity plus jobs partitioning are present; native `build-exe` and `test` keys now include canonical runtime archive path, size, and SHA-256. Project keys now also hash the resolved `zig cc` driver identity and Linux objcopy candidate identity (canonical path, size, SHA-256, and `--version` first line). Ordered native plugin libraries and generated rpath fragments now participate in `build-exe`/`test` keys under schema `sa-build-cache-v4` (path/size/SHA-256, sorted discovery), with focused unit content/extra-library miss coverage and smoke warm-hit publication (`1/1` each). Host link-policy identity (`link_flags`: rpath/system libs/`export_dynamic=default-none`) and expanded host-target identity (object format/pointer width/pinned CPU features policy) also participate, with focused unit evidence `1/1`. Incremental backend ABI v11 covers LLVM, target triple, generic CPU policy, pipeline, and partial-link policy, with focused regressions locking `cpu=generic-v1;features=none`, ordered extra link inputs/host rpath argv placement, and ordered `plugin_import_roots` source-tree hashing (`1/1` each). Focused empty/legacy/malformed/kind/size/missing/extra-file/oversize/same-size content-flip corruption regressions pass `1/1` each. Host link-policy/export-dynamic identity now participates via `link_flags`, and host-target includes object format, pointer width, and pinned CPU/features policy. Complete target-feature matrix expansion, broader failure-injection/TOCTOU/cross-process corruption, authorization beyond hit revalidation, and native macOS/Windows validation remain.
...
- [ ] **P0.3 cache contract**: complete the remaining native/link inputs beyond current `zig cc`, Linux `objcopy`, and ordered plugin library/rpath identity keying, including complete target CPU/features matrix beyond the pinned generic/none policy and remaining non-host link inputs, broader corruption and authorization inputs; broaden path/remaining corruption coverage beyond the focused empty/legacy/malformed/kind/size/missing/extra-file/oversize/same-size-content-flip/symlink guards; inject remaining end-to-end emit/link failures beyond the focused driver `spawn`/`run`, copy/sync/rename/manifest/publish injection, and invalid-entry republish repair; and validate crash recovery, cleanup/lock accounting, same-key cross-process behavior, and native macOS/Windows behavior.
...
- [ ] **P0.6 cache explanation**: first build artifact JSON reason layer plus read-only `sa cache status/why` surface are focused verified (`hit`, `disabled`, `absent`, `dependency_changed`, `manifest_invalid`, `artifact_corrupt`, `incomplete`, `unknown`; status/why additionally explains otherwise reusable `--max-age-days` matches as `expired`; status/why shows kind, key prefix, reason, manifest, bytes, last-write mtime, source-free `last_hit_ns`/`last_store_ns`, source-free `last_store_result`, `last_store_stage`, `last_store_owner_miss_reason`, Linux `last_store_writer_pid`, Linux `last_store_writer_start_ticks`, `last_store_event_count`, `last_store_published_event_count`, and `last_store_failed_event_count` for store events, redacted existing-entry `first_difference` fields, and source-free candidate `first_difference` values for all current key-input fields plus `key.digest` fallback for an absent key with a same-prefix old candidate); writer-pid/start-ticks/event-history/result-count/owner-miss-reason/key-redaction/header-validation evidence passes focused single-flight `1/1`, focused header-validation `1/1`, cache smoke `3/3`, and Debug build `5/5`; `sa test --compile-only --json`, successful ordinary `sa test --json`, `sa test --list --json`, and successful `sa test --affected --json` now expose focused test-cache metrics for cold `absent`, warm `hit`, cold filtered/list `selection_changed`, and disabled list `disabled`; build-exe CGU artifact builds and non-cacheable dynamic-dependency build-exe runs now expose focused `bypassed_untrusted`; ordinary plugin link-input `build-exe`/`test` paths now key and publish under v4 with focused miss/hit evidence instead of bypass; `sa cache clean` removals now leave source-free eviction markers so `sa cache why --json` can distinguish `evicted` from never-seen `absent`; project-cache claim/owner lock failures now expose focused `lock_owner_failed`; text `sa cache why` redaction now covers same-prefix candidate key-input and dynamic-dependency differences; store-event marker/history reads now ignore mismatched `version`/kind/key-prefix telemetry; richer owner lifecycle telemetry, full redaction review beyond the focused store-event/key-prefix/dynamic-dependency/key-input checks, and broader coverage remain.
