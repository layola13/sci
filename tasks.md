...
- [ ] Execute the macOS workflow on both GitHub runners and record native compiler plus `test-runtime-basic`/`test-runtime-darwin`/`test-runtime-darwin-socket` results. Workflow definitions and Linux cross type/object/link/ABI evidence are not macOS native execution or L2 support.
