...
- [x] `std::process` Command builder uid/gid wrapper 批次已完成：新增 `PROCESS_COMMAND_BUILDER_STATUS_UID_GID` / `SPAWN_UID_GID`，把现有 builder 标量状态转发到真实 `PROCESS_*_COMMAND_EXT_UID_GID` runtime 路径，并保留显式 `uid/gid` 与 `has_uid/has_gid` flags。新增 focused 单测并同步 `progress/current_plan/std_missing`。不主张 env maps、pipe Stdio redirection wiring、heap Command object、groups/chroot builder wrappers 或 capture-time uid/gid 集成。
