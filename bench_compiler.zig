...
const emit_llvm = @import("src/emit_llvm.zig");
...
pub fn main() !void {
        ...
        if (entry.kind == .directory) {
            ...
            switch (verified) {
                .ok => |ok| {
                    ...
                    const tmp_ll = "/tmp/bench_tmp.ll";
                    try std.fs.cwd().writeFile(.{ .sub_path = tmp_ll, .data = ll_text });

                    const t4 = std.time.nanoTimestamp();
                    const result = std.process.Child.run(.{
                        .allocator = alloc,
                        .argv = &[_][]const u8{ "zig", "cc", "-c", tmp_ll, "-o", "/dev/null" },
                    }) catch continue;
    ...
    std.debug.print("Emitter   (LLVM IR Gen)   : {d} ms\n", .{total_emit_ns / 1_000_000});
