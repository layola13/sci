...
test "sa_term raw mode and winsize are usable from C" {
    ...
    const c_source =
        ...
        \\    SaTermWinsize unsupported = {.row = 1, .col = 2, .xpixel = 3, .ypixel = 4};
        \\    int pipe_fds[2] = {-1, -1};
        ...
        \\    before.c_iflag |= ICRNL | IXON | IXOFF;
        \\    before.c_oflag |= OPOST;
        \\    before.c_cflag &= ~(CSIZE | PARENB);
        \\    before.c_cflag |= CS8 | CREAD;
        \\    before.c_lflag |= ISIG | ICANON | ECHO | ECHONL | IEXTEN;
        \\    before.c_cc[VMIN] = 7;
        \\    before.c_cc[VTIME] = 3;
        \\    if (tcsetattr(STDIN_FILENO, TCSANOW, &before) != 0) return 6;
        \\    if (tcgetattr(STDIN_FILENO, &before) != 0) return 7;
        \\    if (sa_term_raw_enter(sa_io_stdin(), &session) != SA_STD_OK) return 8;
        \\    if (session == 0) return 9;
        \\    if (tcgetattr(STDIN_FILENO, &after) != 0) return 10;
        \\    if ((after.c_iflag & (IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR | ICRNL | IXON | IXOFF)) != 0) return 11;
        \\    if ((after.c_oflag & OPOST) != 0) return 12;
        \\    if ((after.c_lflag & (ISIG | ICANON | ECHO | ECHONL | IEXTEN)) != 0) return 13;
        \\    if ((after.c_cflag & CSIZE) != CS8 || (after.c_cflag & PARENB) != 0 || (after.c_cflag & CREAD) == 0) return 14;
        \\    if (after.c_cc[VMIN] != 1 || after.c_cc[VTIME] != 0) return 15;
        \\    if (sa_term_winsize(sa_io_stdin(), &queried) != SA_STD_OK) return 16;
        \\    if (queried.row != 24 || queried.col != 80) return 17;
        \\    if (queried.xpixel != 320 || queried.ypixel != 200) return 18;
        \\    if (sa_term_raw_leave(session) != SA_STD_OK) return 19;
        \\    if (sa_term_raw_leave(session) != SA_STD_ERR_INVALID_HANDLE) return 20;
        \\    if (tcgetattr(STDIN_FILENO, &after) != 0) return 21;
        \\    if (!termios_matches(&before, &after)) return 22;
        \\    if (pipe(pipe_fds) != 0) return 23;
        \\    if (sa_std_fd_from_raw(pipe_fds[0], &pipe_handle) != SA_STD_OK || pipe_handle == 0) return 24;
        \\    pipe_fds[0] = -1;
        \\    if (sa_term_winsize(pipe_handle, &unsupported) != SA_STD_ERR_UNSUPPORTED) return 25;
        \\    if (unsupported.row != 0 || unsupported.col != 0 || unsupported.xpixel != 0 || unsupported.ypixel != 0) return 26;
        \\    if (sa_std_close(pipe_handle) != SA_STD_OK) return 27;
        \\    close(pipe_fds[1]);
        \\    close(master);
