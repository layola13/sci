...
- The VecDeque audit has now landed the first eight batches; the latest adds the repeated-value `VEC_DEQUE_TRY_INSERT_N_WITHIN_CAPACITY` helpers. Remaining supportable VecDeque candidates include a concrete `VEC_DEQUE_SPLICE` / drain-range subset (still gated on a ring-buffer aware runtime splice ABI), a `VEC_DEQUE_TRY_INSERT_FROM_SLICE_*_WITHIN_CAPACITY` slice-insert lowering, and a `VEC_DEQUE_EXTEND_CHARS_*` lowering; all intentionally avoid Rust lazy iterator/generic/scoped-reference semantics.
