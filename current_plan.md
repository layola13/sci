...
- Source/facade/test changes are complete for the Vec checked get_mut alias batch.
- Focused source tests for the newly added Vec checked get_mut aliases pass, install sync passes, and focused installed-state tests for the same aliases pass. Full test suites are intentionally not run for this batch per user instruction.
...
- Commit the Vec checked get_mut alias batch, then continue the highest-priority String/Vec Rust API parity audit with only newly added focused tests per batch.
