...
2. [x] Add test-only `spawn`/`run` injection around `zig cc` process wrappers.
3. [x] Route objcopy localization through the same `runProcess` wrapper.
4. [x] Focused verify driver unit tests (`6/6`), format/diff, then commit/push.
...
2. [x] Inject low-level `sync`/`rename` IO failures on store paths with stage telemetry and no partial entries.
3. [x] Open entry directories with `no_follow` and reject symlink entry dirs before nested probes.
4. [x] Reclaim unowned stale entry/build lock files during clean without polluting entry kept/removed stats.
5. [x] Fix symlink test-metadata fixture layout so the real payload stays outside the entry.
6. [x] Focused verify project-cache unit filters (`27/27`), format/diff, then commit/push.
...
5. [ ] **Partial — P0.6 cache explanation surface:**
   ...
   - [x] report `cache.reason="bypassed_untrusted"` for build-exe CGU artifact paths, non-cacheable dynamic-dependency build-exe paths, while ordinary plugin link-input `build-exe`/`test` paths now key/publish under v4 instead of bypass; focused CGU/dynamic-dependency bypass evidence remains;
...
7. [ ] Close the remaining artifact-key boundary:
   - [x] key native `build-exe`/`test` on canonical runtime archive path, size, and SHA-256 under namespace v4;
   - [x] pass an isolated same-path/same-size archive content-flip key test `1/1`;
   - [x] include LLVM version, target triple, generic CPU policy, backend pipeline, and partial-link policy through backend ABI v11 in artifact key v4 and function key v8;
