...
2026-07-17 P0.6 update: source-free per-key store-event history count is now focused verified through `last_store_event_count`, including failed `copy_output` count `1` followed by successful waiter publish count `2`. Linux writer process-start identity is now focused verified through `last_store_writer_start_ticks`. Remaining owner telemetry work now means cross-process owner lifecycle, redaction review, and cross-platform evidence beyond latest result/stage/Linux pid/start-ticks/event count.
