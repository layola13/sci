...
- Added `BTREE_MAP_VALUES_MUT` as the Rust method-name alias over `BTREE_MAP_VALUES_MUT_PTRS`.
- The helper materializes raw `u64` value-slot pointer bits in sorted key order, supports in-place source-map updates, and does not consume the map.
- Test file `std_btree_map_values_mut_alias_macro_surface.sa` (panic ID 10821).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_map_values_mut_alias_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper is eager and exposes raw value-slot pointers. It does not model Rust's lazy `ValuesMut<'a, K, V>` object, scoped mutable references/lifetimes, aliasing guarantees, generic value ABI, B-tree node traversal internals, or panic/drop cleanup.

## Active std parity batch (2026-07-18 BTreeSet ranged extract_if)

Completed supportable half-open ranged eager `std::collections::BTreeSet::extract_if` lowering:

- Added `BTREE_SET_EXTRACT_IF_RANGE` over the existing eager sorted set range materialization.
- The helper applies concrete `[start, end)` slice-key bounds, calls the predicate only for in-range sorted keys, materializes matching keys into a new sorted BTreeSet, and removes those keys from the source after snapshot scanning is complete.
- Test file `std_btree_set_extract_if_range_macro_surface.sa` (panic ID 10820).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_set_extract_if_range_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper models only half-open concrete bounds and eager exhaustion. It does not model Rust's other `RangeBounds` variants, lazy `ExtractIf` object, partial iterator consumption, scoped reference lifetimes, generic `Ord`, allocator cloning, drop repair, or panic/leak cleanup.

## Active std parity batch (2026-07-18 BTreeMap ranged extract_if)

Completed supportable half-open ranged eager `std::collections::BTreeMap::extract_if` lowering:

- Added `BTREE_MAP_EXTRACT_IF_RANGE` over the existing eager mutable-range materialization.
- The helper applies concrete `[start, end)` slice-key bounds, calls the mutable predicate only for in-range sorted entries, preserves predicate writes, materializes matching entries into a new sorted BTreeMap, and removes those entries from the source after range scanning is complete.
- Test file `std_btree_map_extract_if_range_macro_surface.sa` (panic ID 10819).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_map_extract_if_range_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper models only half-open concrete bounds and eager exhaustion. It does not model Rust's other `RangeBounds` variants, lazy `ExtractIf` object, partial iterator consumption, scoped reference lifetimes/aliasing, generic `Ord`, allocator cloning, drop repair, or panic/leak cleanup.

## Active std parity batch (2026-07-18 BTreeMap extract_if)

Completed supportable full-range eager `std::collections::BTreeMap::extract_if` lowering:

- Added `BTREE_MAP_EXTRACT_IF` over the concrete sorted-array `Slice`-key / `u64`-value map.
- The helper eagerly evaluates a concrete `key_slice, value_slot_ptr -> u64` predicate across the full map, preserves predicate writes to both retained and extracted values, materializes matching entries into a new sorted BTreeMap, and stably compacts non-matching entries in the source map.
- Test file `std_btree_map_extract_if_macro_surface.sa` (panic ID 10818).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_map_extract_if_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper is full-range and eager. It does not model Rust's range-bounded lazy `ExtractIf` object, partial iterator consumption, `FnMut(&K, &mut V)` reference/lifetime ABI, generic `RangeBounds` / `Ord` dispatch, allocator cloning, drop repair, or panic/leak cleanup.
...
- Added `BTREE_SET_INTO_ITER` over the existing eager sorted-key iterator materialization and clear helpers.
- The alias materializes ascending key order as flat `Vec<u64>` pairs of `key_ptr_bits, key_len` and clears the source set.
- Test file `std_btree_set_into_iter_alias_macro_surface.sa` (panic ID 10815).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_set_into_iter_alias_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper exposes Rust's owned `IntoIterator` name over concrete eager sorted-key materialization. It does not model Rust's owned `IntoIter` object layout/drop glue, generic item ownership/drop semantics, allocator state, B-tree node traversal internals, lazy consumption, or panic/drop cleanup.
...
- Added `BTREE_MAP_ITER` and `BTREE_MAP_ITER_MUT_PTRS` over the existing concrete sorted-array `Slice`-key / `u64`-value map.
- `BTREE_MAP_ITER` materializes sorted key order as flat `Vec<u64>` triples of `key_ptr_bits, key_len, value`; `BTREE_MAP_ITER_MUT_PTRS` uses `value_slot_ptr_bits` in the third lane for in-place value updates.
- Test file `std_btree_map_iter_alias_macro_surface.sa` (panic ID 10808).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_btree_map_iter_alias_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: these helpers are eager and concrete. They do not model Rust's lazy `Iter` / `IterMut` objects, scoped borrow/reference lifetimes, generic tuple/reference item ABI, B-tree node traversal internals, or panic/drop cleanup.
...
- Added `VEC_DEQUE_SPLICE_U64` over a concrete `u64` deque and replacement slice.
- Wrapped storage is first made contiguous, then spliced through the existing `VEC_SPLICE_U64` helper on a temporary Vec; the deque is rebuilt front-to-back from the spliced result and the drained range is returned in a caller-owned Vec.
- Test file `std_vec_deque_splice_macro_surface.sa` (panic ID 10804).

Focused validation only:

- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_vec_deque_splice_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Scope note: this helper is eager and concrete. It does not model Rust's lazy `Splice` iterator object, generic replacement iterator, allocator access, partial-consumption drop behavior, scoped references, panic objects, or generic item move/drop semantics.
...
- `VEC_DEQUE_ITER_TRY_COLLECT_U64`, `VEC_DEQUE_ITER_COLLECT_INTO_U64`, `VEC_DEQUE_ITER_COPIED_COLLECT_U64`, and `VEC_DEQUE_ITER_CLONED_COLLECT_U64`, plus matching `VEC_DEQUE_INTO_ITER_*` names.
- These helpers consume explicit snapshot/drained backing cursors through concrete collect helpers. `TRY_COLLECT` returns explicit `ok` plus mapped output Vec and consumes the first failing item, `COLLECT_INTO` appends to an existing Vec, and `COPIED`/`CLONED` are value-copy aliases for the concrete `u64` cursor.
- This batch does not model Rust generic `Try` residual conversion, generic `FromIterator` / `Extend` dispatch, borrowed reference item semantics, generic `Copy` / `Clone` trait dispatch, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_vec_deque_iter_collect_ext_macro_surface.sa` (panic ID 10799).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_vec_deque_iter_collect_ext_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
- `VEC_DEQUE_ITER_MAP_COLLECT_U64`, `VEC_DEQUE_ITER_INSPECT_COLLECT_U64`, `VEC_DEQUE_ITER_SCAN_COLLECT_U64`, `VEC_DEQUE_ITER_FILTER_COLLECT_U64`, `VEC_DEQUE_ITER_FILTER_MAP_COLLECT_U64`, and `VEC_DEQUE_ITER_MAP_WHILE_COLLECT_U64`, plus matching `VEC_DEQUE_INTO_ITER_*` names.
- These helpers consume explicit snapshot/drained backing cursors and eagerly materialize concrete `Vec<u64>` outputs, preserving scan/map_while short-circuit suffix behavior.
- This batch does not model Rust lazy adapter identity/state, generic `Option` / `FromIterator` / `Try` conversion, closure capture, allocator-parametric owned iterator layout, reference item identity/lifetimes, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_vec_deque_iter_transform_collect_macro_surface.sa` (panic ID 10794).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_vec_deque_iter_transform_collect_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
- `VEC_DEQUE_ITER_IS_SORTED_U64`, `VEC_DEQUE_ITER_IS_SORTED_BY_U64`, `VEC_DEQUE_ITER_IS_SORTED_BY_KEY_U64`, `VEC_DEQUE_ITER_PARTITION_U64`, and `VEC_DEQUE_ITER_IS_PARTITIONED_U64`, plus matching `VEC_DEQUE_INTO_ITER_*` names.
- These helpers consume explicit snapshot/drained backing cursors and delegate to concrete iterator sortedness/partition helpers.
- This batch does not model Rust generic `PartialOrd` / `Ord` dispatch, closure capture, lazy adapter/object identity, `Default` / `Extend` generic collection dispatch, reference item identity/lifetimes, owned iterator layout, allocator state, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_vec_deque_iter_sorted_partition_macro_surface.sa` (panic ID 10792).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_vec_deque_iter_sorted_partition_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10793+.
...
- `BINARY_HEAP_INTO_ITER_SORTED_ARRAY_CHUNKS_COLLECT_U64` and `BINARY_HEAP_INTO_ITER_SORTED_MAP_WINDOWS_COLLECT_U64` expose Rust forward `Iterator` adapter naming over the materialized descending sorted cursor.
- These helpers eagerly materialize concrete `Vec<u64>` outputs greatest-first. `ARRAY_CHUNKS` leaves an incomplete suffix; `MAP_WINDOWS` uses a concrete window pointer/length callback.
- This batch does not model const-generic array ABI, borrowed window lifetimes, lazy adapter identity/state, generic closure/`FromIterator`, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_array_window_collect_macro_surface.sa` (panic ID 10783).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_array_window_collect_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10784+.
...
- `BINARY_HEAP_INTO_ITER_SORTED_MAP_COLLECT_U64`, `BINARY_HEAP_INTO_ITER_SORTED_FILTER_COLLECT_U64`, `BINARY_HEAP_INTO_ITER_SORTED_FILTER_MAP_COLLECT_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_MAP_WHILE_COLLECT_U64` expose Rust forward `Iterator` transform/filter naming over the materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first and eagerly materialize concrete `Vec<u64>` outputs. `MAP_WHILE` leaves the suffix after the first failed callback available.
- This batch does not model lazy adapter identity/state, generic `Option` / `Try`, closure capture, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_transform_collect_macro_surface.sa` (panic ID 10781).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_transform_collect_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10782+.


## Active std parity batch (2026-07-18 BinaryHeap IntoIterSorted take/skip adapter aliases)

Completed supportable `std::collections::BinaryHeap::IntoIterSorted` eager adapter aliases:
- `BINARY_HEAP_INTO_ITER_SORTED_TAKE_COLLECT_U64` and `BINARY_HEAP_INTO_ITER_SORTED_SKIP_COLLECT_U64` expose Rust forward `Iterator` `take(...).collect` and `skip(...).collect` naming over the materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first and eagerly materialize caller-owned `Vec<u64>` outputs. They intentionally do not add `rev` because Rust `IntoIterSorted` is not a `DoubleEndedIterator`.
- This batch does not model lazy adapter identity/state, generic `IntoIterator` / `FromIterator`, closure capture, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_adapter_collect_macro_surface.sa` (panic ID 10780).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_adapter_collect_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
- `BINARY_HEAP_INTO_ITER_SORTED_MIN_BY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MAX_BY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MIN_BY_KEY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MAX_BY_KEY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_CMP_U64`, `BINARY_HEAP_INTO_ITER_SORTED_CMP_BY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_PARTIAL_CMP_U64`, `BINARY_HEAP_INTO_ITER_SORTED_PARTIAL_CMP_BY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_EQ_BY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_EQ_U64`, `BINARY_HEAP_INTO_ITER_SORTED_PARTIAL_EQ_U64`, `BINARY_HEAP_INTO_ITER_SORTED_NE_U64`, `BINARY_HEAP_INTO_ITER_SORTED_LT_U64`, `BINARY_HEAP_INTO_ITER_SORTED_LE_U64`, `BINARY_HEAP_INTO_ITER_SORTED_GT_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_GE_U64` expose Rust forward `Iterator` comparison/key aggregate naming over the existing materialized descending sorted cursor.
- These helpers consume one or two explicit descending cursors greatest-first. They do not expose reverse traversal because Rust `IntoIterSorted` implements `Iterator` / `ExactSizeIterator`, not `DoubleEndedIterator`.
- This batch does not model Rust generic `Ord` / `PartialOrd` dispatch, closure capture, `Option<Ordering>` ABI, lazy pop-on-next storage, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_comparison_macro_surface.sa` (panic ID 10777).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_comparison_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
## Active std parity batch (2026-07-17 BinaryHeap IntoIterSorted fold/aggregate aliases)

Completed supportable `std::collections::BinaryHeap::IntoIterSorted` fold/aggregate aliases:
- `BINARY_HEAP_INTO_ITER_SORTED_FOLD_U64`, `BINARY_HEAP_INTO_ITER_SORTED_TRY_FOLD_U64`, `BINARY_HEAP_INTO_ITER_SORTED_SUM_U64`, `BINARY_HEAP_INTO_ITER_SORTED_PRODUCT_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MIN_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MAX_U64`, `BINARY_HEAP_INTO_ITER_SORTED_REDUCE_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_TRY_REDUCE_U64` expose Rust forward `Iterator` naming over the existing materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first. They do not expose reverse fold methods because Rust `IntoIterSorted` implements `Iterator` / `ExactSizeIterator`, not `DoubleEndedIterator`.
- This batch does not model Rust closure capture, generic `Try` residual conversion, `Sum` / `Product` / `Ord` trait dispatch, Rust `Option<T>` ABI, lazy pop-on-next storage, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_fold_aggregate_macro_surface.sa` (panic ID 10776).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_fold_aggregate_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10777+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIterSorted any/all/for_each aliases)

Completed supportable `std::collections::BinaryHeap::IntoIterSorted` any/all/for_each aliases:
- `BINARY_HEAP_INTO_ITER_SORTED_ANY_U64`, `BINARY_HEAP_INTO_ITER_SORTED_ALL_U64`, `BINARY_HEAP_INTO_ITER_SORTED_FOR_EACH_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_TRY_FOR_EACH_U64` expose Rust forward `Iterator` naming over the existing materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first. They do not expose reverse traversal because Rust `IntoIterSorted` implements `Iterator` / `ExactSizeIterator`, not `DoubleEndedIterator`.
...
- Test file `std_binary_heap_into_iter_sorted_any_all_for_each_macro_surface.sa` (panic ID 10775).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_any_all_for_each_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
- `BINARY_HEAP_INTO_ITER_SORTED_TRY_NEXT_CHUNK_U64` and `BINARY_HEAP_INTO_ITER_SORTED_ADVANCE_BY` expose Rust forward `Iterator` default method naming over the existing materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first. They do not expose `advance_back_by` or other reverse cursor methods because Rust `IntoIterSorted` implements `Iterator` / `ExactSizeIterator`, not `DoubleEndedIterator`.
- This batch does not model Rust `array::IntoIter` remainder ABI, `NonZero<usize>` layout, const-generic array item ABI, lazy pop-on-next storage, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_advance_chunk_macro_surface.sa` (panic ID 10773).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_advance_chunk_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10774+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIterSorted cursor ops aliases)

Completed supportable `std::collections::BinaryHeap::IntoIterSorted` cursor/terminal operation aliases:
- `BINARY_HEAP_INTO_ITER_SORTED_NTH_U64`, `BINARY_HEAP_INTO_ITER_SORTED_COUNT_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_LAST_U64` expose Rust forward `Iterator` default method naming over the existing materialized descending sorted cursor.
- These helpers consume the explicit descending cursor greatest-first. They do not expose reverse cursor methods because Rust `IntoIterSorted` implements `Iterator` / `ExactSizeIterator`, not `DoubleEndedIterator`.
- This batch does not model Rust `Option<T>` ABI, allocator-parametric owned iterator layout, lazy pop-on-next storage, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_sorted_cursor_ops_macro_surface.sa` (panic ID 10772).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_cursor_ops_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10773+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter for_each aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` for_each aliases:
- `BINARY_HEAP_INTO_ITER_FOR_EACH_U64` and `BINARY_HEAP_INTO_ITER_TRY_FOR_EACH_U64` expose Rust owned iterator `for_each` / `try_for_each` naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order. `try_for_each` short-circuits after the callback returns false and leaves the suffix after the failing item available.
- This batch does not model Rust closure capture, generic `Try` residual conversion, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_for_each_macro_surface.sa` (panic ID 10771).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_for_each_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10772+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter any/all aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` any/all aliases:
- `BINARY_HEAP_INTO_ITER_ANY_U64` and `BINARY_HEAP_INTO_ITER_ALL_U64` expose Rust owned iterator `any` / `all` naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order and return explicit bool scalars. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust closure capture, generic `Try` residual conversion, `Option` ABI, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_any_all_macro_surface.sa` (panic ID 10770).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_any_all_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10771+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter search aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` search/predicate aliases:
- `BINARY_HEAP_INTO_ITER_FIND_U64`, `BINARY_HEAP_INTO_ITER_TRY_FIND_U64`, `BINARY_HEAP_INTO_ITER_FIND_MAP_U64`, `BINARY_HEAP_INTO_ITER_POSITION_U64`, `BINARY_HEAP_INTO_ITER_RPOSITION_U64`, and `BINARY_HEAP_INTO_ITER_RFIND_U64` expose Rust owned iterator search naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order, or from the back for reverse variants. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust closure capture, generic `Try` residual conversion, `Option` ABI, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_search_macro_surface.sa` (panic ID 10769).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_search_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10770+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter advance/chunk aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` advance/chunk aliases:
- `BINARY_HEAP_INTO_ITER_TRY_NEXT_CHUNK_U64`, `BINARY_HEAP_INTO_ITER_ADVANCE_BY`, and `BINARY_HEAP_INTO_ITER_ADVANCE_BACK_BY` expose Rust owned iterator `next_chunk`, `advance_by`, and `advance_back_by` naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order, or from the back for `advance_back_by`. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust `array::IntoIter` remainder ABI, `NonZero<usize>` layout, const-generic array item ABI, allocator-parametric owned iterator layout, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_advance_chunk_macro_surface.sa` (panic ID 10768).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_advance_chunk_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10769+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter cursor ops aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` cursor/terminal operation aliases:
- `BINARY_HEAP_INTO_ITER_NTH_U64`, `BINARY_HEAP_INTO_ITER_NTH_BACK_U64`, `BINARY_HEAP_INTO_ITER_COUNT_U64`, and `BINARY_HEAP_INTO_ITER_LAST_U64` expose Rust owned iterator `nth`, `nth_back`, `count`, and `last` naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order, or from the back for `nth_back`. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust `Option<T>` ABI, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_cursor_ops_macro_surface.sa` (panic ID 10767).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_cursor_ops_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10768+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter aggregate aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` aggregate/reduce aliases:
- `BINARY_HEAP_INTO_ITER_SUM_U64`, `BINARY_HEAP_INTO_ITER_PRODUCT_U64`, `BINARY_HEAP_INTO_ITER_MIN_U64`, `BINARY_HEAP_INTO_ITER_MAX_U64`, `BINARY_HEAP_INTO_ITER_REDUCE_U64`, and `BINARY_HEAP_INTO_ITER_TRY_REDUCE_U64` expose Rust owned iterator terminal aggregate/reduce naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust `Sum` / `Product` / `Ord` trait dispatch, generic `Try` residual conversion, `Option` ABI, allocator-parametric owned iterator layout, or generic item semantics.
- Test file `std_binary_heap_into_iter_aggregate_macro_surface.sa` (panic ID 10766).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_aggregate_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10767+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter fold aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` fold aliases:
- `BINARY_HEAP_INTO_ITER_FOLD_U64`, `BINARY_HEAP_INTO_ITER_TRY_FOLD_U64`, `BINARY_HEAP_INTO_ITER_RFOLD_U64`, and `BINARY_HEAP_INTO_ITER_TRY_RFOLD_U64` expose Rust owned iterator fold naming over the existing Vec-backed consuming heap cursor.
- These helpers consume the explicit cursor in heap-array backing order, or from the back for reverse folds. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust closure capture, generic `Try` residual conversion, allocator-parametric owned iterator layout, generic item move/drop semantics, or panic/drop cleanup.
- Test file `std_binary_heap_into_iter_fold_macro_surface.sa` (panic ID 10765).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_fold_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10766+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap IntoIter comparison aliases)

Completed supportable `std::collections::BinaryHeap::IntoIter` comparison/key aggregate aliases:
- `BINARY_HEAP_INTO_ITER_MIN_BY_U64`, `BINARY_HEAP_INTO_ITER_MAX_BY_U64`, `BINARY_HEAP_INTO_ITER_MIN_BY_KEY_U64`, `BINARY_HEAP_INTO_ITER_MAX_BY_KEY_U64`, `BINARY_HEAP_INTO_ITER_CMP_U64`, `BINARY_HEAP_INTO_ITER_CMP_BY_U64`, `BINARY_HEAP_INTO_ITER_PARTIAL_CMP_U64`, `BINARY_HEAP_INTO_ITER_PARTIAL_CMP_BY_U64`, `BINARY_HEAP_INTO_ITER_EQ_BY_U64`, `BINARY_HEAP_INTO_ITER_EQ_U64`, `BINARY_HEAP_INTO_ITER_PARTIAL_EQ_U64`, `BINARY_HEAP_INTO_ITER_NE_U64`, `BINARY_HEAP_INTO_ITER_LT_U64`, `BINARY_HEAP_INTO_ITER_LE_U64`, `BINARY_HEAP_INTO_ITER_GT_U64`, and `BINARY_HEAP_INTO_ITER_GE_U64` expose Rust `Iterator` comparison and key aggregate naming over the existing Vec-backed consuming heap cursor.
- These helpers consume one or two explicit cursors in heap-array backing order. The heap itself has already been consumed into caller-owned backing storage.
- This batch does not model Rust generic `Ord` / `PartialOrd` dispatch, closure capture, `Option<Ordering>` ABI, allocator-parametric owned iterator layout, or generic item move/drop semantics.
- Test file `std_binary_heap_into_iter_comparison_macro_surface.sa` (panic ID 10764).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_comparison_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10765+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.

## Active std parity batch (2026-07-17 BinaryHeap Iter search aliases)

Completed supportable `std::collections::BinaryHeap::Iter` search/predicate aliases:
- `BINARY_HEAP_ITER_FIND_U64`, `BINARY_HEAP_ITER_TRY_FIND_U64`, `BINARY_HEAP_ITER_FIND_MAP_U64`, `BINARY_HEAP_ITER_POSITION_U64`, `BINARY_HEAP_ITER_RPOSITION_U64`, and `BINARY_HEAP_ITER_RFIND_U64` expose Rust borrowed iterator search naming over the existing slice-backed heap cursor.
- These helpers consume only the cursor in internal heap-array order, or from the back for reverse variants; they do not consume or reorder the heap itself.
...
- Test file `std_binary_heap_iter_search_macro_surface.sa` (panic ID 10759).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_search_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_any_all_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
## Active std parity batch (2026-07-17 BinaryHeap Iter fold aliases)

Completed supportable `std::collections::BinaryHeap::Iter` fold aliases:
- `BINARY_HEAP_ITER_FOLD_U64`, `BINARY_HEAP_ITER_TRY_FOLD_U64`, `BINARY_HEAP_ITER_RFOLD_U64`, and `BINARY_HEAP_ITER_TRY_RFOLD_U64` expose Rust borrowed iterator fold naming over the existing slice-backed heap cursor.
- These helpers consume only the cursor in internal heap-array order, or from the back for reverse folds; they do not consume or reorder the heap itself.
- This batch does not model Rust closure capture, generic `Try` residual conversion, reference item lifetimes, or panic/drop cleanup.
- Test file `std_binary_heap_iter_fold_macro_surface.sa` (panic ID 10763).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_fold_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
Completed supportable `std::collections::BinaryHeap::Iter` advance/chunk aliases:
- `BINARY_HEAP_ITER_TRY_NEXT_CHUNK_U64`, `BINARY_HEAP_ITER_ADVANCE_BY`, and `BINARY_HEAP_ITER_ADVANCE_BACK_BY` expose Rust borrowed iterator `next_chunk`, `advance_by`, and `advance_back_by` naming over the existing slice-backed heap cursor.
- These helpers consume only the cursor in internal heap-array order; they do not consume or reorder the heap itself.
- This batch does not model Rust `array::IntoIter` remainder ABI, `NonZero<usize>` layout, const-generic array item ABI, reference item lifetimes, or panic/drop cleanup.
- Test file `std_binary_heap_iter_advance_chunk_macro_surface.sa` (panic ID 10758).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_advance_chunk_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10759+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.


## Active std parity batch (2026-07-17 BinaryHeap Iter cursor ops aliases)

Completed supportable `std::collections::BinaryHeap::Iter` cursor/terminal operation aliases:
- `BINARY_HEAP_ITER_NTH_U64`, `BINARY_HEAP_ITER_NTH_BACK_U64`, `BINARY_HEAP_ITER_COUNT_U64`, and `BINARY_HEAP_ITER_LAST_U64` expose Rust borrowed iterator `nth`, `nth_back`, `count`, and `last` naming over the existing slice-backed heap cursor.
- These helpers consume only the cursor in internal heap-array order; they do not consume or reorder the heap itself.
- This batch does not model Rust reference item lifetimes, generic element semantics, `Option<&T>` ABI, or panic/drop cleanup.
- Test file `std_binary_heap_iter_cursor_ops_macro_surface.sa` (panic ID 10757).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_cursor_ops_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.

Panic IDs next free: 10758+.
Still blocked without redesign: generic trait impl dispatch, Rust lifetime/borrow semantics, Rust reference item identity, generic primitive/container trait dispatch, cross-width integer conversions, generic `NonZero<T>`, generic `Wrapping<T>` / `Saturating<T>`, `u128` / `i128` public primitive support, portable target-width `usize` / `isize` switching beyond the current explicit 64-bit ABI, missing concrete wrapper widths, Rust feature-gate modeling, Rust niche optimization, Rust unsafe UB enforcement, Rust panic message/object behavior, parser/formatter trait integration, allocator/drop/borrow semantics, randomized `RandomState`, SipHash compatibility, Rust iterator trait hierarchy, generic `Option` / `Result` / `ControlFlow`, generic `FromIterator`, `IntoIterator`, true format!, Condvar/Barrier, process env maps / Stdio redirection, path component iterators, pre_exec closure ABI, thread stack/name builder ABI, and stdio lock guard handle modeling.


## Active std parity batch (2026-07-17 BinaryHeap Iter collect aliases)

Completed supportable `std::collections::BinaryHeap::Iter` collect aliases:
- `BINARY_HEAP_ITER_COLLECT_U64`, `BINARY_HEAP_ITER_TRY_COLLECT_U64`, and `BINARY_HEAP_ITER_COLLECT_INTO_U64` expose Rust borrowed iterator materializing collection naming over the existing slice-backed heap cursor.
- These helpers consume the cursor in internal heap-array order into caller-owned `Vec<u64>` outputs; they do not consume or reorder the heap itself.
- This batch does not model Rust reference item lifetimes, generic `FromIterator` / `Extend` / `Try` residual conversion, `Result` / `Option` ABI, or generic element semantics.
- Test file `std_binary_heap_iter_collect_macro_surface.sa` (panic ID 10756).

Focused validation only:
- `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_iter_collect_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
## Active std parity batch (2026-07-16 Vec into_iter helper)

Completed supportable defaults/aliases/macros:
- Concrete consuming Vec iterator helper in vec.sa: VEC_INTO_ITER_U64, consuming the source Vec into an explicit backing Vec and returning a slice-backed Iter cursor over that Vec.
- Test file std_vec_into_iter_macro_surface.sa (panic ID 10707).

Panic IDs next free: 10708+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state, lazy drain/splice adapters, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter default helper)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter default helper in vec.sa: VEC_INTO_ITER_DEFAULT_U64, creating an empty backing Vec and empty slice-backed Iter cursor for Rust's `Default for vec::IntoIter` shape.
- Test file std_vec_into_iter_default_macro_surface.sa (panic ID 10708).

Panic IDs next free: 10709+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state, lazy drain/splice adapters, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter slice views)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter remaining-slice aliases in vec.sa: VEC_INTO_ITER_AS_SLICE_U64 and VEC_INTO_ITER_AS_MUT_SLICE_U64, exposing the current remaining cursor range as a Slice view over the explicit backing Vec.
- Test file std_vec_into_iter_slice_macro_surface.sa (panic ID 10709).
...
## Active std parity batch (2026-07-16 Vec IntoIter min/max-by ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter comparator/key aliases in vec.sa: VEC_INTO_ITER_MIN_BY_U64, VEC_INTO_ITER_MAX_BY_U64, VEC_INTO_ITER_MIN_BY_KEY_U64, and VEC_INTO_ITER_MAX_BY_KEY_U64, delegating to existing concrete u64 cursor comparator/key aggregate helpers over the explicit backing Vec.
- Test file std_vec_into_iter_min_max_by_macro_surface.sa (panic ID 10717).
...
## Active std parity batch (2026-07-16 Vec IntoIter collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter materializing collect aliases in vec.sa: VEC_INTO_ITER_COLLECT_U64, VEC_INTO_ITER_TRY_COLLECT_U64, and VEC_INTO_ITER_COLLECT_INTO_U64, delegating to existing concrete u64 cursor collect helpers over explicit backing Vecs.
- Test file std_vec_into_iter_collect_macro_surface.sa (panic ID 10720).

Panic IDs next free: 10721+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state/cloning, generic FromIterator dispatch, generic Try residual conversion, Rust Result/Option ABI, lazy adapter identity, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter adapter collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter eager adapter collect aliases in vec.sa: VEC_INTO_ITER_TAKE_COLLECT_U64, VEC_INTO_ITER_SKIP_COLLECT_U64, VEC_INTO_ITER_REV_COLLECT_U64, VEC_INTO_ITER_TRY_STEP_BY_COLLECT_U64, and VEC_INTO_ITER_STEP_BY_COLLECT_U64, delegating to existing concrete u64 cursor adapter collect helpers over explicit backing Vecs.
- Test file std_vec_into_iter_adapter_collect_macro_surface.sa (panic ID 10721).

Panic IDs next free: 10722+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state/cloning, lazy adapter object identity/state, generic Iterator<Item=T>, panic object behavior, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter transform collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter transform/filter eager collect aliases in vec.sa: VEC_INTO_ITER_MAP_COLLECT_U64, VEC_INTO_ITER_INSPECT_COLLECT_U64, VEC_INTO_ITER_SCAN_COLLECT_U64, VEC_INTO_ITER_FILTER_COLLECT_U64, VEC_INTO_ITER_FILTER_MAP_COLLECT_U64, and VEC_INTO_ITER_MAP_WHILE_COLLECT_U64, delegating to existing concrete u64 cursor transform/filter collect helpers over explicit backing Vecs.
- Test file std_vec_into_iter_transform_collect_macro_surface.sa (panic ID 10722).

Panic IDs next free: 10723+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state/cloning, lazy adapter object identity/state, closure capture, generic Option/Iterator<Item=T>, panic/drop cleanup, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter sequence collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter sequence eager collect aliases in vec.sa: VEC_INTO_ITER_INTERSPERSE_COLLECT_U64, VEC_INTO_ITER_INTERSPERSE_WITH_COLLECT_U64, VEC_INTO_ITER_CYCLE_TAKE_COLLECT_U64, VEC_INTO_ITER_TAKE_WHILE_COLLECT_U64, and VEC_INTO_ITER_SKIP_WHILE_COLLECT_U64, delegating to existing concrete u64 cursor sequence collect helpers over explicit backing Vecs.
- Test file std_vec_into_iter_sequence_collect_macro_surface.sa (panic ID 10723).

Panic IDs next free: 10724+.
Still blocked without redesign: generic Vec<T> iterator item/reference/lifetime semantics, Rust owned IntoIter<T,A> object layout/drop glue, allocator-aware iterator state/cloning, lazy adapter object identity/state, infinite Cycle behavior, generic closure capture, panic/drop cleanup, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-16 Vec IntoIter pair collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Vec IntoIter pair/composition eager collect aliases in vec.sa: VEC_INTO_ITER_ENUMERATE_COLLECT_U64, VEC_INTO_ITER_ZIP_COLLECT_U64, and VEC_INTO_ITER_CHAIN_COLLECT_U64, delegating to existing concrete u64 cursor pair/composition collect helpers over explicit backing Vecs.
- Test file std_vec_into_iter_pair_collect_macro_surface.sa (panic ID 10724).
...
## Active std parity batch (2026-07-17 BinaryHeap Iter trait/clone)

Completed supportable defaults/aliases/macros:
- Concrete BinaryHeap borrowed Iter clone/cursor/size aliases in binary_heap.sa: BINARY_HEAP_ITER_CLONE_U64, BINARY_HEAP_ITER_NEXT_U64, BINARY_HEAP_ITER_NEXT_BACK_U64, BINARY_HEAP_ITER_SIZE_HINT_U64, BINARY_HEAP_ITER_EXACT_SIZE_LEN_U64, and BINARY_HEAP_ITER_EXACT_SIZE_IS_EMPTY_U64.
- Test file std_binary_heap_iter_trait_clone_macro_surface.sa (panic ID 10733).

Panic IDs next free: 10734+.
Still blocked without redesign: generic BinaryHeap<T> borrowed reference item semantics, Rust lifetime enforcement, iterator object layout/debug formatting, marker trait wiring, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-17 Vec Iter trait/clone)

Completed supportable defaults/aliases/macros:
- Concrete Vec borrowed Iter as-ref/clone/cursor/size aliases in vec.sa: VEC_ITER_AS_REF_SLICE_U64, VEC_ITER_CLONE_U64, VEC_ITER_NEXT_U64, VEC_ITER_NEXT_BACK_U64, VEC_ITER_SIZE_HINT_U64, VEC_ITER_EXACT_SIZE_LEN_U64, and VEC_ITER_EXACT_SIZE_IS_EMPTY_U64.
- Test file std_vec_iter_trait_clone_macro_surface.sa (panic ID 10734).
...
## Active std parity batch (2026-07-17 Array IntoIter fold ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter fold aliases in array.sa: ARRAY_INTO_ITER_FOLD_U64, ARRAY_INTO_ITER_TRY_FOLD_U64, ARRAY_INTO_ITER_RFOLD_U64, and ARRAY_INTO_ITER_TRY_RFOLD_U64.
- Test file std_array_into_iter_fold_macro_surface.sa (panic ID 10739).
...
## Active std parity batch (2026-07-17 Array IntoIter comparator/key aggregate ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter comparator/key aggregate aliases in array.sa: ARRAY_INTO_ITER_MIN_BY_U64, ARRAY_INTO_ITER_MAX_BY_U64, ARRAY_INTO_ITER_MIN_BY_KEY_U64, and ARRAY_INTO_ITER_MAX_BY_KEY_U64.
- Test file std_array_into_iter_min_max_by_macro_surface.sa (panic ID 10741).

Panic IDs next free: 10742+.
Still blocked without redesign: Rust `Option<T>` ABI for comparator/key aggregates, generic `Ord` trait dispatch, generic closure capture/call ABI, generic item move/drop semantics, const-generic array object layout, marker trait wiring, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-17 Array IntoIter search/predicate ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter search/predicate aliases in array.sa: ARRAY_INTO_ITER_FIND_U64, ARRAY_INTO_ITER_TRY_FIND_U64, ARRAY_INTO_ITER_FIND_MAP_U64, ARRAY_INTO_ITER_POSITION_U64, ARRAY_INTO_ITER_RPOSITION_U64, ARRAY_INTO_ITER_RFIND_U64, ARRAY_INTO_ITER_ANY_U64, and ARRAY_INTO_ITER_ALL_U64.
- Test file std_array_into_iter_search_macro_surface.sa (panic ID 10742).
...
## Active std parity batch (2026-07-17 Array IntoIter sortedness ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter sortedness aliases in array.sa: ARRAY_INTO_ITER_IS_SORTED_U64, ARRAY_INTO_ITER_IS_SORTED_BY_U64, and ARRAY_INTO_ITER_IS_SORTED_BY_KEY_U64.
- Test file std_array_into_iter_sorted_macro_surface.sa (panic ID 10744).
...
## Active std parity batch (2026-07-17 Array IntoIter collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter materializing collect aliases in array.sa: ARRAY_INTO_ITER_COLLECT_U64, ARRAY_INTO_ITER_TRY_COLLECT_U64, and ARRAY_INTO_ITER_COLLECT_INTO_U64.
- Test file std_array_into_iter_collect_macro_surface.sa (panic ID 10746).

Panic IDs next free: 10747+.
Still blocked without redesign: Rust generic `FromIterator` / `Extend` dispatch, generic `Try` residual conversion, Rust `Result` / `Option` ABI, allocator-parametric collection, generic item move/drop semantics, const-generic array object layout, marker trait wiring, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-17 Array IntoIter adapter collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter eager adapter collect aliases in array.sa: ARRAY_INTO_ITER_TAKE_COLLECT_U64, ARRAY_INTO_ITER_SKIP_COLLECT_U64, ARRAY_INTO_ITER_REV_COLLECT_U64, ARRAY_INTO_ITER_TRY_STEP_BY_COLLECT_U64, and ARRAY_INTO_ITER_STEP_BY_COLLECT_U64.
- Test file std_array_into_iter_adapter_collect_macro_surface.sa (panic ID 10747).
...
## Active std parity batch (2026-07-17 Array IntoIter sequence collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter sequence eager collect aliases in array.sa: ARRAY_INTO_ITER_INTERSPERSE_COLLECT_U64, ARRAY_INTO_ITER_INTERSPERSE_WITH_COLLECT_U64, ARRAY_INTO_ITER_CYCLE_TAKE_COLLECT_U64, ARRAY_INTO_ITER_TAKE_WHILE_COLLECT_U64, and ARRAY_INTO_ITER_SKIP_WHILE_COLLECT_U64.
- Test file std_array_into_iter_sequence_collect_macro_surface.sa (panic ID 10749).
...
## Active std parity batch (2026-07-17 Array IntoIter adapter-state collect ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter adapter-state eager collect aliases in array.sa: ARRAY_INTO_ITER_BY_REF_COLLECT_U64, ARRAY_INTO_ITER_FUSE_COLLECT_U64, and ARRAY_INTO_ITER_PEEKABLE_COLLECT_U64.
- Test file std_array_into_iter_adapter_state_collect_macro_surface.sa (panic ID 10751).

Panic IDs next free: 10752+.
Still blocked without redesign: Rust lazy `ByRef` / `Fuse` / `Peekable` adapter identity/state types, borrow lifetime/reference semantics, generic `Iterator<Item = T>` object composition, generic item move/drop semantics, const-generic array object layout, marker trait wiring, true format!, Condvar/Barrier, process env maps/Stdio objects, path component iterators, thread stack/name builder ABI.

## Active std parity batch (2026-07-17 Array IntoIter partition ops)

Completed supportable defaults/aliases/macros:
- Concrete Array owned IntoIter partition aliases in array.sa: ARRAY_INTO_ITER_PARTITION_U64 and ARRAY_INTO_ITER_IS_PARTITIONED_U64.
- Test file std_array_into_iter_partition_macro_surface.sa (panic ID 10752).
