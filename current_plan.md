...
Active follow-up: make full-test diagnostics usable before any further broad performance work. The previous compiler-performance slice reduced SCI `sa test --filter` / `--list` latency for large SAB artifacts generated by downstream `sla_ecs` to the target range and is installed. The current priority is to prevent another 10-20 minute blind aggregate test run by making each full-test dependency print its owner, timeout, elapsed time, and summary.
