...
5. [ ] **Partial — P0.6 cache explanation surface:**
   ...
   - [ ] verify source-free Linux writer identity in `.sa_cache/.store-events/<kind>/<key>` and expose it as `last_store_writer_pid` in `sa cache status/why`; legacy/missing fields report `null`, focused single-flight/cache smoke and Debug build are pending;
   ...
   - [ ] finish fine-grained candidate old-entry key input first-difference reporting beyond coarse `key.digest`, richer write-event history/owner telemetry beyond result/stage/writer pid, redaction review, and broader cross-process/cross-platform coverage.
