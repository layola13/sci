...
Active compiler-performance slice: reduce SCI `sa test --filter` / `--list` latency for large SAB artifacts generated by downstream `sla_ecs`, targeting a path toward about 1 second for focused tests. Keep existing `sa_std` String/Vec work committed before this slice; current baseline commit is `ee50937` (`Add extended string macro surfaces`).
...
1. Active performance issue:
   ...
   - First fix target: add a `.sab` `sa test --list` fast path that decodes test metadata/function signatures without full verifier. Next target after that: focused compile path should prune/reachability-filter selected tests before verify/LLVM emit/link, with cache-key handling for filtered test selections.
