...
Active follow-up: reduce the slowest full-test runtime owners using the logged step runner, without returning to blind aggregate test runs. The previous compiler-performance slice reduced SCI `sa test --filter` / `--list` latency for large SAB artifacts generated by downstream `sla_ecs` to the target range and is installed. The test logging milestone is complete and committed, so optimization now proceeds step-by-step from the measured slowest owners.
