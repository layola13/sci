...
Active compiler-performance slice: reduce SCI `sa test --filter` / `--list` latency for large SAB artifacts generated by downstream `sla_ecs`, targeting about 1 second for focused compile-only/list iteration. Keep existing `sa_std` String/Vec work committed before this slice; current baseline commit is `ee50937` (`Add extended string macro surfaces`) and the first SCI SAB checkpoint is `94d841c` (`Optimize SAB test listing path`).
