...
fn cliErrorInfo(err: anyerror) CliErrorInfo {
    return switch (err) {
        ...
        error.LlvmDisNotFound => .{ .code = "SA-CLI-016", .message = "LLVM bitcode disassembler not found", .hint = "install llvm-dis-14/llvm-dis or clang and make one of them available on PATH before running bc2sa" },
