...
const StreamWriterCtx = struct {
    stream: plugin_api.HostStream,
};

fn streamWriteFn(ctx: *const anyopaque, bytes: []const u8) anyerror!usize {
...
fn makeAnyWriter(stream: plugin_api.HostStream, storage: *StreamWriterCtx) ?std.io.AnyWriter {
...
fn runSaxCommandAbi(ctx: *const plugin_api.Context, argv: [*]const [*:0]const u8, argv_len: usize, stdout: plugin_api.HostStream, stderr: plugin_api.HostStream, out_code: *u8) callconv(.c) u32 {
    ...
    var stdout_storage: StreamWriterCtx = undefined;
    var stderr_storage: StreamWriterCtx = undefined;
    const stdout_writer = makeAnyWriter(stdout, &stdout_storage) orelse return @intFromEnum(plugin_api.AbiStatus.failed);
    const stderr_writer = makeAnyWriter(stderr, &stderr_storage) orelse return @intFromEnum(plugin_api.AbiStatus.failed);
