...
- **Partial — artifact key/cache contract:** the namespace is now v3 with compiler/Zig, host target, jobs, and semantic file inputs. Native `build-exe`/`test` keys include the canonical `sa_std` archive path, size, and SHA-256, while obj/wasm correctly have no runtime archive input. The project key now also hashes the resolved `zig cc` driver identity and Linux objcopy candidate identity (canonical path, size, SHA-256, and `--version` first line). The isolated same-path/same-size content-flip key test passes `1/1`, and focused toolchain-identity tests pass for both fake `zig` and Linux fake `llvm-objcopy`. Build-exe/test plugin link inputs and their generated rpath flags are currently treated as non-cacheable and report `bypassed_untrusted`, with focused evidence that no build-exe/test entry is published. Incremental backend ABI v11 already covers LLVM, target triple, generic CPU policy, pipeline, and partial-link policy, and focused regressions now lock `cpu=generic-v1;features=none`, ordered extra link inputs/host rpath argv placement, and ordered `plugin_import_roots` source-tree hashing (`1/1` each). Full ordered plugin/export/rpath/link flag key publication beyond the current bypass/argv/order regressions; complete target-feature matrix; corruption; authorization; and native macOS/Windows validation remain.
