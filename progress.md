...
- Defined the first native Windows x86_64 CI gate from the portability MVP checklist (native execution still pending):
  ...
  - `build.zig` now exposes `test-portable`, a filtered `test-runtime-windows` subset, and `sa-cli`; the runtime step refuses non-Windows x86_64 host/target combinations so Linux cross builds cannot be reported as native evidence.
  ...
  - Static contract gate passes `3/3`; YAML parses; clean Linux worktree `portable-host-typecheck` passes `8/8`. This host has no `pwsh` or Windows runner, so the workflow and PowerShell smoke have not been executed natively and Windows L2 remains unclaimed.
