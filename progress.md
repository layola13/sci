...
- **Pending P0:** unified metrics (P0.1), remaining verify snapshot/source-map/differential/pressure contract (P0.2), final depfile/native-link key and cache concurrency contract (P0.3), request-local daemon paths/peer identity/bounded queue (P0.4), one shared reachability index (P0.5), explainable miss tooling (P0.6), provenance-complete baseline and 22-step rerun (P0.7), and backend A/B profiling (P0.8).
...
- Verification rule: only the named focused tests are evidence. The final v11 snapshot passes the incremental/safety/emitter subsets and fresh Debug build recorded above, in addition to the previously recorded same-output publication, single-flight owner-handoff, OOM no-partial-publication, dynamic-dependency, and cache-closure checks. Full `zig fmt --check src tests` still reports untouched pre-existing files, so no repository-wide format claim is made. Failed two-file publication repair, failure injection, broader corruption, cross-process/cross-platform behavior, and cross-consumer reachability gates remain.
