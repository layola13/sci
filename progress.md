...
- **P0.6 boundary:** this is still not full cache explanation. True candidate old-entry key first-difference reporting, richer write-event audit telemetry beyond a coarse successful/failed result, full redaction review, and cross-process/cross-platform evidence remain open. The current `expired` reason is a read-only status/why policy, not build lookup or eviction semantics; the current `evicted` reason is limited to `sa cache clean` removals; the current `lock_owner_failed` evidence is limited to deterministic project-cache lock-path failure; ordinary test-cache metrics are verified only for successful cold/warm `sa test --json` after the runner exits, affected metrics are verified only for successful cold/warm `--affected --json` test runs, list metrics are verified only for JSON cold/disabled/hit paths, plugin bypass evidence is Linux-focused, dynamic-dependency bypass evidence uses a test-only same-request-change hook, and store-event evidence covers only coarse successful publication and failed store attempts without writer identity, failure stage taxonomy, event history, or cross-process owner details.
