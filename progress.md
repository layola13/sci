...
- **Focused verified — P0.6 candidate compiler/toolchain key-input differences:** the focused status/why smoke now verifies `first_difference="key.compiler"` and `first_difference="key.toolchain"` for absent requested keys when same-prefix source-free sidecars first differ in compiler or toolchain inputs. The JSON output still redacts the full key and both sidecar digests.
