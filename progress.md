...
- **Focused verified — P0.6 candidate key-digest difference:** `sa cache why --json` now reports `first_difference="key.digest"` for an absent requested key when the same cache kind contains a source-free old candidate with the same 12-character key prefix but a different complete key. The focused cache status/why smoke verifies the absent reason, redacted key prefix, and absence of the full candidate key in JSON output. Focused cache smoke remains `13/13`, and Debug `sa-cli` build passes `5/5`.
