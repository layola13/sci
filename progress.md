...
- **Focused verified — P0.3 project artifact regular-file guard:** project cache artifact/output validation now rejects symlink entries before accepting manifest size/SHA-256 matches. The symlink regression constructs an `output.bin` symlink whose target matches the manifest, and lookup still returns `incomplete` with `first_difference="output.file"`. Focused gate passes: `project cache manifest rejects symlink artifact entries` `1/1`; `zig fmt --check src/cli.zig`; `git diff --check`.
