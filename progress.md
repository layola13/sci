...
- Native plugin libraries discovered through `SA_PLUGINS_PATH` / installed plugin roots are sorted by canonical path; each library contributes path/size/SHA-256 plus `-Wl,-rpath,<dir>` identity.
- `appendNativePluginLinkInputs` now sorts selected libraries before linking, and build-exe/test no longer force `bypassed_untrusted` solely because plugin link inputs are present. CGU and non-cacheable dynamic-dependency bypasses remain.
- Focused validation passed:
  - `zig test ... --test-filter "project build key tracks ordered plugin link inputs"` -> `1/1`
  - `zig test ... --test-filter "plugin link inputs participate in project cache keys"` -> `1/1` (cold miss → warm hit for build-exe/test; extra unused plugin library forces miss)
  - `zig test ... --test-filter "project cache manifest rejects kind mismatch entries"` -> `1/1`
  - `zig test ... --test-filter "project cache manifest rejects artifact size mismatch as corrupt"` -> `1/1`
  - `zig test ... --test-filter "project cache manifest rejects missing output as incomplete"` -> `1/1`
  - `zig fmt --check src/cli.zig tests/cli_smoke.zig`; `git diff --check`; `zig build sa-cli --summary all` -> `5/5`
- Evidence boundary: this is Linux-focused ordered plugin library/rpath key publication and hit/miss evidence. Export-symbol matrices, complete target-feature policy, broader corruption/failure injection, authorization completion, and native macOS/Windows validation remain open.

## Focused verified: 2026-07-17 ordered plugin link inputs in artifact key v4

- Project artifact keys for `build-exe` and `test` now include ordered native plugin libraries from `SA_PLUGINS_PATH` / installed plugin roots: canonical path, size, SHA-256, and generated `-Wl,-rpath,...` fragments under schema `sa-build-cache-v4`.
- Plugin discovery order is sorted before hashing and before native linking so key/link identity is stable.
- Ordinary `build-exe`/`test` no longer force `bypassed_untrusted` merely because plugin libraries are present; reusable entries publish when other cacheability conditions hold.
- Focused validation passed on Linux:
  - unit: `project build key tracks ordered plugin link inputs` -> `1/1`
  - unit: kind-mismatch / size-mutation / missing-output / empty / legacy-v1 / malformed-digest corruption regressions -> `1/1` each
  - smoke: `plugin link inputs participate in project cache keys` -> `1/1` (cold miss, warm hit, extra unused plugin miss, test cold miss→warm hit)
  - `zig fmt --check src/cli.zig tests/cli_smoke.zig`; `git diff --check`
- Evidence boundary: this is Linux unit+smoke coverage for ordered plugin/rpath key publication. Export flags, complete target-feature matrix, failure injection, TOCTOU-hard path authorization, crash recovery, and native macOS/Windows validation remain open. CGU and non-cacheable dynamic-dependency paths still report `bypassed_untrusted`.

## Focused verified: 2026-07-17 project cache kind/size/missing corruption regressions

- Added focused project-cache corruption regressions for kind-mismatched manifests, post-publish artifact size mutations, and missing output files.
- Focused validation passed:
  - focused `src/cli.zig` filters for kind mismatch / artifact size mismatch / missing output -> `1/1` each
  - `zig fmt --check src/cli.zig`
- Evidence boundary: this extends Linux unit coverage for incomplete/corrupt/invalid project-cache entries. It does not claim failure injection, crash recovery, TOCTOU-hard path authorization, or native macOS/Windows validation.
...
- Added Linux-runnable regressions that lock three remaining artifact-key boundary fragments:
  - `src/driver/zigcc.zig`: ordered extra inputs and host rpath placement in `argvForExe` / `argvForWasm`.
  - `src/emit_llvm_llvmc.zig`: backend cache identity pins `cpu=generic-v1;features=none`, legacy pipeline, and OS-specific partial-link policy under ABI v11.
  - `src/cli.zig`: source-tree hash changes when `plugin_import_roots` order is reversed.
- Focused validation passed:
  - `zig test src/driver/zigcc.zig -ODebug --test-filter "argv helpers preserve input and link flag order"` -> `1/1`
  - `zig test src/emit_llvm_llvmc.zig -ODebug -lc -isystem /usr/lib/llvm-14/include -L /usr/lib/llvm-14/lib -lLLVM-14 src/emit_llvm_llvmc_shim.c --test-filter "backend cache identity pins cpu"` -> `1/1`
  - `zig test src/cli.zig -ODebug -lc --test-filter "source tree hash keeps plugin import root order"` -> `1/1`
- Evidence boundary: this is Linux unit-regression coverage for ordered plugin/link argv shape, pinned CPU/features policy identity, and ordered plugin import-root hashing. It does not claim full ordered plugin/export/rpath key publication, complete target-feature matrix coverage, corruption/authorization completion, or native macOS/Windows runner execution.
...
- A mistaken aggregate `zig build test -- test-filter ...` run was interrupted after unrelated Linux/macOS/PAL contract steps started; it is not claimed as a pass. The previously unstaged `src/driver/zigcc.zig` and `src/emit_llvm_llvmc.zig` order/policy regressions are now verified and included in the artifact-key boundary batch.
...
- The macOS/Windows source contracts require the success-only upload condition and reject `if: always()` in the native workflows.
- Validation passed on Linux: `zig fmt --check tests/macos_native_ci_contract.zig tests/windows_native_ci_contract.zig`; Python YAML parsing for macOS/Windows native workflows; `zig build macos-ci-contract --summary all` -> macOS contract `3/3` plus validator `8/8`; `zig build windows-ci-contract --summary all` -> Windows contract `5/5` plus validator `8/8`; `git diff --check`.
- Evidence boundary: this prevents ambiguous failed-run evidence artifacts in future native workflows, but it remains workflow/source-contract evidence until macOS/Windows runners execute the updated workflows.
...
- The macOS/Windows source contracts lock the matrix validator and expected target strings, and validator tests cover macOS target drift, Windows non-native target drift, and unsupported Windows `arm64` smoke evidence.
- Validation passed on Linux: `zig fmt --check tools/ci/validate_native_evidence.zig tests/macos_native_ci_contract.zig tests/windows_native_ci_contract.zig`; `zig build native-evidence-validator --summary all` -> `7/7`; `zig build macos-ci-contract --summary all` -> macOS contract `3/3` plus validator `7/7`; `zig build windows-ci-contract --summary all` -> Windows contract `5/5` plus validator `7/7`; `zig build release-contract --summary all` -> release contract `7/7` plus validator `7/7`; `git diff --check`.
- Evidence boundary: this is Linux validator/source-contract hardening until macOS/Windows runners execute the updated workflows.
...
- `tools/ci/validate_native_evidence.zig` validates the schema version before platform-specific smoke/runtime fields and includes a schema-drift rejection test.
- The macOS/Windows source contracts lock the runtime evidence field, smoke JSON field, and shared validator rule.
- Evidence boundary: this pins the future native artifact schema, but it remains source-contract evidence until the native GitHub runners execute the updated workflows.
...
- `tools/ci/windows_native_smoke.ps1` now packages its staged release payload as `sa-windows-<arch>.zip`, extracts it to a separate clean directory, and runs the same compiler/package smoke from the extracted archive layout.
...
- Linux-runnable helper coverage verifies prefix-derived masks, MAC formatting, and structured JSON escaping. Windows x86_64/aarch64 PAL type checks and PE test links validate the ABI declarations against Zig 0.14.1's Windows target headers.
- Actual static-runtime PE linking exposed the existing IOCP `AcceptEx` import-library gap; the Windows runtime link set now also includes `mswsock` alongside `ws2_32` and the new `iphlpapi` dependency.
...
- A shared POSIX PAL helper uses target-native `std.c.sockaddr.in/in6` layouts and structured JSON encoding. Linux preserves sysfs MAC lookup and adds synthetic sockaddr plus real enumeration tests. macOS uses Darwin address offsets and `AF_LINK/sockaddr_dl` MAC bytes. Windows remains explicitly unsupported pending a native `GetAdaptersAddresses` backend and native runner evidence.
- Validation passed: `test-runtime-pal pal-source-contract pal-typecheck portable-runtime-typecheck` (`45/45`, `test-runtime-pal` `11/11`), `test-runtime-basic sa-std-abi sa-std-artifact-abi` (`25/25`), `portability-check` (`65/65`), ReleaseFast (`13/13`), and `git diff --check`.
- The unrelated cache-writer PID diagnostics remain unstaged.
...
- **Focused verified — P0.2 VerificationInputDigest v2 verdict-only checkpoint:** `src/verification_input.zig` now computes a schema-namespaced digest over full verdict inputs: instruction structure including expanded order and package identity/SHA, const values including vtables, canonical package grants/capability masks, SAX component, metadata mode plus predecoded symbols/signatures, and `check_exit_leaks`. `src/incr_verify.zig` is a success-only boolean verdict map with explicit consumer capability, process-local lifetime, and a coarse 4096-entry cap. `src/verifier.zig` exposes `verifyInput()` and `verifyVerdictOnly()`; Trap results are not recorded. `sa check` now returns a check-only result with metrics, uses `verify-verdict-v2` for text and SAB miss→hit, and SAB check passes predecoded metadata instead of delegating to normal compile. Compile/build/emit still run the full Referee and the repeated compile containment gate confirms annotations/deltas/gas are preserved with zero verdict hits.
- **Focused P0.2 evidence:** `verification_input` `24/24`, `incr_verify` `26/26`, verifier trap-not-cached `1/1`, SAB check miss→hit `1/1`, text check miss→hit CLI smoke `1/1`, repeated compile/codegen containment `1/1`, related `zig fmt --check`, `git diff --check`, and fresh Debug `sa-cli` build `5/5`. A direct full `src/referee.zig` test run was interrupted after 161/187 with no failure output and is not claimed as a pass.
- **P0.2 boundary:** this checkpoint does not add persistent verdict cache, owned `VerifySnapshot` restore, source-map rebind, complete cache-on/off field differential, daemon/cross-process pressure, cross-platform validation, or formal P50/P95/RSS data. Verdict-only hits remain restricted to pass/fail `check` consumers; compile/emit and any path needing annotations, delta, gas, symbols, signatures, or source maps must re-run Referee.
...
- **Focused verified — P0.3 dynamic dependencies:** `src/flattener.zig`/`src/cli.zig` now record environment presence/value digests and canonical included-file path/size/digests through recursive include, carry the request-local depfile in `FlattenResult`, serialize it in manifest v2, and prevalidate it before publishing a hit. This replaces the blanket source-text bypass while retaining a non-cacheable result if a dependency changes during capture. Focused results: INCLUDE `2/2`, absent `OPTION_ENV!` `1/1`, file/environment validator `1/1`, INCLUDE_STR cache miss→hit→content-flip miss `1/1`, and explicit `OPTION_ENV!` absent miss→hit→present miss→hit `1/1`. The final entry count remains one, and its manifest contains `present:true` plus the exact value SHA-256 without storing the raw value.
- **Focused verified — P0.3 nested relative include depfile closure:** the project artifact cache now has CLI smoke evidence for `INCLUDE!` of a subdirectory fragment that performs relative `INCLUDE_STR!`. The first build publishes one `build-exe` entry, a warm rebuild hits it, and changing `nested/payload.txt` rejects the old entry with `cache.reason="dependency_changed"` while the manifest records both `fragment.sa` and `payload.txt`. Focused dynamic-dependency cache gates pass `3/3`: top-level INCLUDE_STR, nested relative INCLUDE/INCLUDE_STR, and OPTION_ENV absent-to-present.
- **Focused verified — P0.3 whole-entry publication and compile single-flight:** project artifacts publish from a sibling random staging directory only after artifact/output/test metadata are synced and manifest v2 is written last. Per-key entry locks pin hit/store/cleanup activity, while separate blocking build-owner locks serialize cacheable compilation and permit one waiter to take over after an owner fails even after copying the artifact. Build exe/obj/wasm compile and restore into private sibling output stages before final publication, preventing different cache keys that share `-o` from contaminating stored entries. Different keys publishing to the same final path are serialized by a persistent `.sa-output-locks/<basename>` file lock; same-filesystem stage files rename artifact-first and output-last, so successful concurrent publishers cannot leave an A/B pair. Cache claim/store failures degrade to ordinary compilation/best-effort population for build and test commands. Direct same-output pairing, owner-handoff, and OOM no-partial-entry tests pass `1/1` each; prior concurrency/cleanup/dynamic-shape tests pass `4/4`, and INCLUDE_STR/cache-clean/test-cache CLI closures pass `3/3`. Output-last is a success commit marker, not a power-loss/crash-safe two-file transaction.
- **Focused verified — P0.3 incremental function-object integrity subset:** incremental manifest schema v2 authorizes only its exact ordinary, nonempty object path after matching size and SHA-256. A per-key exclusive lock covers lookup, miss emission, linking, manifest replacement, and cleanup. Misses are emitted through synced sibling temporary objects; a successful link precedes atomic manifest replacement, the manifest is committed last, and stale/temp cleanup occurs only after that commit. If object publication fails, the current request links the validated transient object without replacing the old manifest. `DT_UNKNOWN` entries use a metadata fallback that rejects the tested stable symlink. Non-cacheable dynamic dependencies neither reuse nor publish function objects, and object records carry their verified size/digest into manifest writing.
- **Focused verified — function-shard lowering and ownership correctness:** function-object key v8 separates prehashed global lowering context from per-function local context. Body `.reg` operands and verifier `change.reg` entries hash numeric local slots; global symbols, labels, functions, and signature identities remain canonically name-based. DCE assigns process globals to the first shard that is actually emitted and includes that owner bit in the key. Every shard retains the complete signature order, preserving indirect-call provenance indices. Raw quoted strings are emitted only by their owning function shard and receive collision-safe anonymous LLVM names that avoid all declared constant, vtable, and function names. Backend ABI v11 identifies LLVM, target triple, generic CPU policy, pipeline, and partial-link policy. Linux partial links use `objcopy --localize-hidden` to give internal symbols true local binding. Direct and indirect owned-pointer returns are freed according to tracked register/signature type; indirect borrowed returns do not acquire an extra free.
- **Focused Linux evidence for this checkpoint:** incremental CLI `10/10`; `DT_UNKNOWN` regular-file fallback `1/1`; non-cacheable dynamic dependencies `1/1`; split-module emitter `2/2`; local-slot owned-pointer delta `1/1`; anonymous-name collision avoidance `1/1`; related `zig fmt --check` and `git diff --check`; fresh Debug `-j1` build passed.
- **Recorded primary commands:**

  ```bash
  env PATH=/opt/zig:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
  /opt/zig/zig test src/emit_llvm_llvmc_shim.c \
  -lLLVM-14 -ODebug -isystem /usr/lib/llvm-14/include \
  -L /usr/lib/llvm-14/lib -ldl \
  --dep saasm --dep build_options \
  -Mroot=tests/cli_smoke.zig \
  --dep build_options -Msaasm=src/lib.zig \
  -Mbuild_options=.zig-cache/c/50acbea58f5c1376bc7ceb516b0544a2/options.zig \
  -lc --test-filter "incremental"

  env PATH=/opt/zig:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
  /opt/zig/zig build -Doptimize=Debug -j1 \
  --cache-dir /tmp/sci-incremental-debug-v11-final-20260715
  ```
- **Incremental boundary:** this focused integrity result is not full P0.3, Phase 0, or Phase 2 completion and contains no P50/P95/RSS or at-least-100-function performance evidence. A cold miss remains approximately `O(F·I + F²)`: each missed function still scans the verified stream and constructs the complete declaration table, followed by a final whole-module bitcode emit. Key v8 is not complete alpha-normalization because raw text and some function-local parameter/register names remain inputs. The `DT_UNKNOWN` metadata fallback is not TOCTOU-hard `openat`/`O_NOFOLLOW` authorization. Linux now has a hard PATH dependency on `objcopy`; absence is an explicit failure. macOS/Windows retain namespaced hidden strong symbols but have no native validation. Failure injection, crash recovery, broad corruption, same-key cross-process/cross-platform coverage, remaining link-flag/target-policy/authorization keying, and ModuleIndex remain open. The full 119-test emitter run was stopped after `83/119`, and no complete emitter or `llvmc-test` pass is claimed.
- **Focused verified — P0.5 LLVM reachability:** `src/emit_llvm_llvmc.zig` now builds function body ranges once, scans each queued reachable body at most once, and uses one shared `buildParallelEmitTasks()` selection path for in-memory bitcode and bitcode+object artifact emission. Unknown/invalid/indirect/address-taken edges and signature/body count mismatch still disable pruning. Focused gates pass `6/6`: direct closure, self-recursive root single-scan, bitcode emit pruning, artifact emit pruning, indirect/invalid/unknown fallback, and function-operand/address fallback. Production `sa-cli` Debug build passes `5/5`. The broader module test was manually interrupted in unrelated long-running Flattener PBT after the relevant emit/reachability tests had passed, so no full-module test pass is claimed. The shared cross-consumer ReachabilityEngine remains incomplete.
- **Focused verified — P0.6 artifact-cache reason JSON:** `src/cli.zig` now keeps the existing `cache.kind`/`cache.hit` success metrics and adds optional `cache.reason` for project artifact cache paths. Build-exe/build-obj/build-wasm hits report `hit`; misses can now report `absent`, `dependency_changed`, `manifest_invalid`, `artifact_corrupt`, `incomplete`, or `unknown` from lookup/manifest validation instead of collapsing all cases to an unexplained miss. Focused smoke coverage passes `4/4`, including cold miss, warm hit, INCLUDE_STR content flip, OPTION_ENV absent-to-present, artifact corruption repair, and manifest-invalid repair.
- **Focused verified — P0.6 cache status/why surface:** `sa cache status` now scans project-local `.sa_cache` entries and `sa cache why --kind <kind> --key <hex>` explains one entry. Text and JSON output include kind, key prefix, reusable/miss reason, manifest status, bytes, and last-write mtime while avoiding source text, package secrets, absolute private paths, and full digests. Focused cache smoke now passes `10/10`, including the new hit/incomplete/artifact-corrupt/manifest-invalid status and why coverage.
- **Focused verified — P0.6 disabled reason:** `--no-incremental` build-exe/build-obj/build-wasm runs now still emit project artifact cache metrics with `hit=false` and `reason=disabled`, matching the optimization doc's population constraint instead of omitting the cache object. The existing build-exe disabled smoke assertion is updated, and the focused cache smoke remains `10/10`.
- **Focused verified — P0.6 status/why expired reason:** `sa cache status/why --max-age-days <n>` now explains otherwise reusable project-local entries older than the requested age as `expired`, while invalid/corrupt/incomplete entries keep their underlying reason. Focused cache smoke remains `10/10`, including text and JSON expired coverage.
- **Focused verified — P0.6 recent-hit telemetry:** project cache hits now update a source-free `.sa_cache/.hits/<kind>/<key>` marker, and `sa cache status/why` reports that mtime as `last_hit_ns`. The marker lives outside the entry directory, so hit telemetry does not mutate the manifest, artifact files, or entry `last_write_ns`. Focused cache smoke remains `10/10`, and Debug `sa-cli` build remains `5/5`.
- **Focused verified — P0.6 entry first-difference field:** `sa cache status/why` now includes redacted `first_difference` field names for existing invalid/incomplete/corrupt entries, such as `manifest.version`, `output.file`, and `output.size`, while hit/expired entries report `null`. It does not expose env names, paths, hashes, source, or full keys. Focused cache smoke remains `10/10`, and Debug `sa-cli` build remains `5/5`.
- **Focused verified — P0.6 recent-store telemetry:** successful project cache publication now updates a source-free `.sa_cache/.stores/<kind>/<key>` marker, and `sa cache status/why` reports that mtime as `last_store_ns`. The marker lives outside the entry directory, so store telemetry does not mutate the manifest, artifact files, or entry `last_write_ns`. Focused cache smoke remains `10/10`, and Debug `sa-cli` build remains `5/5`.
- **Focused verified — P0.6 test compile-only cache metrics:** `sa test --compile-only --json` now emits the shared success metrics cache object for the project test cache. Cold compile-only population reports `cache.kind="test"`, `hit=false`, and `reason="absent"`; warm compile-only reuse reports `hit=true` and `reason="hit"`. Focused cache smoke remains `10/10`, and Debug `sa-cli` build remains `5/5`.
- **Focused verified — P0.6 ordinary test-run cache metrics:** successful ordinary `sa test --json` now emits the shared success metrics cache object only after the test runner exits with code `0`. Cold test-cache population reports `cache.kind="test"`, `hit=false`, and `reason="absent"`; warm reuse reports `hit=true` and `reason="hit"`. Focused cache smoke remains `10/10`, `zig fmt --check src/cli.zig tests/cli_smoke.zig` passes, `git diff --check` passes, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 selection/bypass reasons:** cold `sa test --compile-only --json --filter ...` now reports `cache.kind="test"`, `hit=false`, and `reason="selection_changed"` when a selected artifact is intentionally not published as the full test cache. Build-exe CGU artifact builds now report `reason="bypassed_untrusted"` when the CGU path intentionally avoids publishing a project artifact-cache entry. Focused cache smoke remains `10/10`, and Debug `sa-cli` build remains `5/5`.
- **Focused verified — P0.6 eviction reason:** `sa cache clean` now writes source-free `.sa_cache/.evictions/<kind>/<key>` markers only after successfully removing real 64-hex project-cache entry directories. `sa cache why --json` reports a removed key as `reason="evicted"`, `manifest="missing"`, and `bytes=0`, while a never-seen key remains `reason="absent"` and valid remaining entries are not marked. Focused cache smoke remains `10/10`, and Debug `sa-cli` build now passes `5/5`.
- **Focused verified — P0.6 lock-owner failure reason:** project-cache claim failures now fall back to ordinary compilation with `cache.hit=false` and `cache.reason="lock_owner_failed"` instead of collapsing to `unknown`. The focused smoke injects a deterministic `.sa_cache/build-exe/.locks` path failure and verifies the JSON reason. Focused cache smoke remains `10/10`, `zig fmt --check src/cli.zig tests/cli_smoke.zig` passes, `git diff --check` passes, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 test plugin bypass reason:** `sa test --json` now has focused smoke coverage for plugin link inputs that intentionally bypass project test-cache publication. A minimal installed plugin exports the referenced extern, both cold runs report `cache.kind="test"`, `hit=false`, and `reason="bypassed_untrusted"`, and no `.sa_cache/test` entry is published. Focused cache smoke now passes `11/11`, `zig fmt --check src/cli.zig tests/cli_smoke.zig` passes, `git diff --check` passes, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 dynamic-dependency bypass reason:** non-cacheable dynamic dependencies now have focused build-exe JSON evidence. A test-only recorder hook simulates a dependency changing during one request, the full `sa build-exe --json` path reports `cache.kind="build-exe"`, `hit=false`, and `reason="bypassed_untrusted"` on repeated runs, and no `.sa_cache/build-exe` entry is published. Focused cache smoke now passes `12/12`, `zig fmt --check src/cli.zig src/flattener.zig tests/cli_smoke.zig` passes, `git diff --check` passes, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 successful store-event audit:** successful project cache publication now writes a source-free `.sa_cache/.store-events/<kind>/<key>` marker alongside the existing `.stores` mtime marker, and `sa cache status/why` reports it as `last_store_result="published"`. The marker is outside the entry directory, so it does not mutate manifest validation, artifact files, or entry `last_write_ns`. Focused cache smoke remains `12/12`, `zig fmt --check src/cli.zig src/flattener.zig tests/cli_smoke.zig` passes, `git diff --check` passes, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 failed store-event audit:** failed project cache store attempts now write a source-free `.sa_cache/.store-events/<kind>/<key>` marker with `last_store_result="failed"`, even when no cache entry directory exists. `sa cache why --json` reports that failed result for the absent key, and a later successful publication overwrites the marker with `published`. Focused single-flight owner-handoff smoke passes `1/1`, focused cache smoke remains `13/13`, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 store-event stage audit:** project cache store events now include a source-free `last_store_stage` enum. The focused failed-owner smoke verifies a missing output failure as `last_store_result="failed"` and `last_store_stage="copy_output"` in `sa cache why --json`, then verifies the later successful handoff overwrites the stage to `publish`. Focused single-flight owner-handoff smoke passes `1/1`.
- **Focused verified — P0.6 store-event writer identity:** project cache store events now record a source-free process id on Linux, macOS, and Windows and `sa cache status/why` exposes it as `last_store_writer_pid`. Existing event markers without this field remain backward-compatible and report `null`; focused single-flight owner-handoff smoke passes `1/1`, focused cache smoke passes `13/13`, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 store-event writer start identity:** project cache store events now record Linux source-free `/proc/self/stat` process start ticks and expose them as `last_store_writer_start_ticks`. Existing markers without this field report `null`; focused single-flight evidence verifies failed and later successful store-event surfaces are non-null on Linux.
- **Focused verified — P0.6 store-event history count:** project cache store events now append source-free per-key history records under `.sa_cache/.store-event-history/<kind>/<key>/`, and `sa cache status/why` exposes the count as `last_store_event_count`. The focused single-flight owner-handoff smoke now verifies a failed `copy_output` store attempt is visible as count `1`, then the waiter publish overwrites the latest event to `published/publish` while preserving a visible history count of `2`.
- **Focused verified — P0.6 store-event history result counts:** `sa cache status/why` now exposes source-free `last_store_published_event_count` and `last_store_failed_event_count` derived from per-key store-event history. The focused single-flight owner-handoff smoke verifies `0/1` after the failed owner and `1/1` after the waiter publish.
- **Focused verified — P0.6 store-event owner miss reason:** project cache store events now carry source-free `owner_miss_reason` and `sa cache status/why` exposes it as `last_store_owner_miss_reason`. The focused single-flight owner-handoff smoke verifies the first failed owner and later waiter publish both report `absent`; older/direct store events remain compatible and report `null`.
- **Focused verified — P0.6 store-event key redaction:** the focused single-flight `sa cache why --json` checks now verify both failed and later successful store-event surfaces expose only the 12-character `key_prefix` and do not include the full 64-character cache key.
- **Focused verified — P0.6 candidate key-input first-difference coverage:** the focused status/why smoke now verifies every current source-free key-input field name for absent requested keys with same-prefix old candidates: `key.schema`, `key.compiler`, `key.backend`, `key.toolchain`, `key.host_target`, `key.project`, `key.command`, `key.wasm_target`, `key.source_tree`, `key.semantic_file_inputs`, and `key.project_manifests`. The JSON output still redacts the full key and both sidecar digests.
- **Focused verified — P0.6 candidate key-input difference:** `sa cache why --json` now reports `first_difference="key.command"` for an absent requested key when the same cache kind contains a source-free old candidate with the same 12-character key prefix but a different recorded request-side key-input trace. The focused cache status/why smoke verifies the absent reason, redacted key prefix, and absence of the full candidate key in JSON output. Focused cache smoke remains `13/13`, and Debug `sa-cli` build passes `5/5`.
- **Focused verified — P0.6 candidate source-tree key-input difference:** the same focused status/why smoke now also verifies `first_difference="key.source_tree"` when same-prefix source-free sidecars agree on command inputs but differ on the recorded source-tree digest. The JSON output still exposes only the redacted key prefix and field name; full keys and the recorded sidecar digests remain hidden. `zig fmt --check tests/cli_smoke.zig` passes, and `PATH=/root/projects/zig-compiler:$PATH /root/projects/zig-compiler/zig build cli-cache-smoke --summary all` passes `3/3`.
- **Focused verified — P0.6 candidate semantic-file key-input difference:** the focused status/why smoke now verifies `first_difference="key.semantic_file_inputs"` when same-prefix sidecars agree on command/source-tree inputs but differ on semantic file inputs. The JSON output still redacts the full key and both sidecar digests.
- **Focused verified — P0.6 candidate project-manifest key-input difference:** the focused status/why smoke now verifies `first_difference="key.project_manifests"` when same-prefix sidecars agree on command, semantic-file, and source-tree inputs but differ on project manifest inputs. The JSON output still redacts the full key and both sidecar digests.
- **Focused verified — P0.6 text why redaction:** the focused status/why smoke now also checks the text `sa cache why` path for same-prefix candidate key-input and dynamic-dependency differences. Text output stays on the redacted 12-character key prefix, still reports the same `first_difference` values, and does not expose the full key, env names, env values, or sidecar digests.
- **Focused verified — P0.6 key-input sidecar shape validation:** `sa cache why --json` now accepts fine-grained same-prefix key-input differences only when both source-free sidecars match the expected version, kind, key prefix, known unique field names, and 64-hex digests. Malformed or cross-key sidecars fall back to `first_difference="key.digest"` without exposing the recorded field digests. A dedicated `cli-cache-smoke` build step now runs the focused project-cache CLI smoke subset, and `PATH=/root/projects/zig-compiler:$PATH /root/projects/zig-compiler/zig build cli-cache-smoke --summary all` passes `3/3`.
- **Focused verified — P0.6 test list JSON cache metrics:** `sa test --list --json` now emits the shared success metrics cache object for project test-cache lookup paths. A cold list query that intentionally does not publish a full test artifact reports `cache.kind="test"`, `hit=false`, and `reason="selection_changed"`; disabled list lookup reports `reason="disabled"`; listing from an existing test-cache entry reports `hit=true` and `reason="hit"`. Focused cache smoke remains `12/12`.
- **Focused verified — P0.6 affected test-run cache metrics:** successful `sa test --affected --json` now has focused cold/warm project test-cache evidence. The first successful affected run reports `cache.kind="test"`, `hit=false`, and `reason="absent"` after executing the selected tests; the second successful run reports `hit=true` and `reason="hit"`. The affected analysis JSON remains on stdout, while success metrics remain on stderr with the existing test diagnostics convention. Focused affected smoke passes `1/1`, focused cache smoke now passes `13/13`, and Debug `sa-cli` build passes `5/5`.
- **P0.6 boundary:** this is still not full cache explanation. Fine-grained candidate old-entry key input first-difference reporting beyond the coarse `key.digest` field is now focused verified for all current source-free key-input fields, and the text `sa cache why` path is now also covered for same-prefix key-input and dynamic-dependency redaction; richer owner lifecycle telemetry beyond latest result/stage/owner miss reason/history result counts/Linux writer pid/start-ticks/event count, full redaction review beyond the focused store-event/key-prefix/dynamic-dependency/key-input checks, and cross-process/cross-platform evidence remain open. The current `expired` reason is a read-only status/why policy, not build lookup or eviction semantics; the current `evicted` reason is limited to `sa cache clean` removals; the current `lock_owner_failed` evidence is limited to deterministic project-cache lock-path failure; ordinary test-cache metrics are verified only for successful cold/warm `sa test --json` after the runner exits, affected metrics are verified only for successful cold/warm `--affected --json` test runs, list metrics are verified only for JSON cold/disabled/hit paths, plugin bypass evidence is Linux-focused, dynamic-dependency bypass evidence uses a test-only same-request-change hook, and store-event owner/history evidence currently covers only source-free owner miss reason, per-key event counts, per-key history result counts, latest coarse successful/failed result, pipeline stage, Linux writer pid/start-ticks, and source-free key-input provenance without broader cross-process owner lifecycle details.
- **Partial — artifact key/cache contract:** the namespace is now v4 with compiler/Zig, host target, jobs, semantic file inputs, and ordered native plugin library/rpath identity for `build-exe`/`test`. Native `build-exe`/`test` keys include the canonical `sa_std` archive path, size, and SHA-256, while obj/wasm correctly have no runtime archive input. The project key also hashes the resolved `zig cc` driver identity and Linux objcopy candidate identity (canonical path, size, SHA-256, and `--version` first line). Focused plugin-key unit/smoke coverage passes content flip and extra-library miss, and warm hit after publication. Incremental backend ABI v11 already covers LLVM, target triple, generic CPU policy, pipeline, and partial-link policy, and focused regressions lock `cpu=generic-v1;features=none`, ordered extra link inputs/host rpath argv placement, and ordered `plugin_import_roots` source-tree hashing (`1/1` each). Focused empty/legacy/malformed/kind/size/missing corruption regressions pass `1/1` each. Export flags, complete target-feature matrix, broader corruption/failure-injection, authorization beyond current hit revalidation, and native macOS/Windows validation remain.
- **Focused verified — P0.3 project cache regular-file guard:** project cache artifact/output validation now rejects symlink entries before accepting manifest size/SHA-256 matches. Manifest lookup/status/first-difference now also reject symlinked `manifest.json`, and cached test metadata rejects symlinked `test-metadata.json` before parsing. Focused gates pass: `project cache manifest rejects symlink artifact entries` `1/1`; `project cache manifest rejects symlink manifest entries` `1/1`; `project cache manifest rejects symlink test metadata entries` `1/1`; `zig fmt --check src/cli.zig`; `git diff --check`.
- **Pending P0:** unified metrics (P0.1), remaining verify snapshot/source-map/differential/pressure contract (P0.2), final native-link key and cache concurrency/failure contract (P0.3), request-local daemon paths/peer identity/bounded queue (P0.4), one shared reachability index (P0.5), remaining candidate-diff/cache telemetry explanation tooling (P0.6), provenance-complete baseline and 22-step rerun (P0.7), and backend A/B profiling (P0.8).
...
- Verification rule: only the named focused tests are evidence. The final v11 snapshot passes the incremental/safety/emitter subsets, the focused P0.5 reachability/emit-path gates, the focused P0.6 artifact-cache reason/status/why/test-compile-only/ordinary-test-run/list/affected/selection/build-exe-CGU-bypass/dynamic-dependency-bypass/test-plugin-bypass/eviction/lock-owner-failure/store-event-success/store-event-failure/store-event-stage/store-event-writer-pid/store-event-history-result-count/key.schema/key.compiler/key.backend/key.toolchain/key.host_target/key.project/key.command/key.wasm_target/key.source_tree/key.semantic_file_inputs/key.project_manifests smoke gates, the focused project artifact/output/manifest/test-metadata symlink rejection gates, and the fresh Debug builds recorded above, in addition to the previously recorded same-output publication, single-flight owner-handoff, OOM no-partial-publication, dynamic-dependency, and cache-closure checks. Full `zig fmt --check src tests` still reports untouched pre-existing files, so no repository-wide format claim is made. Failed two-file publication repair, failure injection, broader corruption, cross-process/cross-platform behavior, remaining P0.6 owner lifecycle/redaction/cross-platform work, and cross-consumer reachability gates remain.
...
- Completed the release/installer artifact-contract batch:
  - `.github/workflows/release.yml` now pins tag builds through `SA_VERSION=$GITHUB_REF_NAME`, uploads every archive with its sidecar, downloads all successful matrix artifacts, strictly validates sidecars, regenerates an archive-only aggregate, and publishes tar/zip/sidecar/checksum files. Non-Linux release matrix entries remain disabled.
  - `tools/release.sh` builds only the compiler/static-runtime payload into an isolated prefix, preserves shared Zig caches and the worktree install prefix, verifies the staged and extracted `bin/` + `std/` layout, and fails on missing archives, tools, payloads, sidecars, or aggregate mismatches.
  - shell, PowerShell, and remote installers use `layola13/sci`; compiler release checksums are mandatory; remote macOS naming uses `macos`; archive std lookup uses `std`; `install.sh` no longer depends on GNU `find -maxdepth` on macOS.
  - Linux evidence: `release-contract` passes `4/4`; POSIX shell syntax checks pass; a real x86_64 archive and both checksum files verify; clean HTTP installation succeeds; installed `sa 0.0.4` passes `check`, builds Hello World from the packaged runtime, and the binary prints `hello, saasm`; missing and mismatched sidecars exit `1` before installing `bin/sa`.
  - Evidence boundary: this Linux host has no `pwsh`, `actionlint`, macOS runner, or Windows runner. PowerShell parsing/execution, GitHub Actions behavior, native zip/tar installation, and clean-machine macOS/Windows smoke remain unverified.
- Defined the first native Windows x86_64 CI gate from the portability MVP checklist (native execution still pending):
  - `.github/workflows/windows-native.yml` targets `windows-2025`, pins Zig `0.14.1` and the LLVM `14.0.6` win64 installer SHA-256, checks `LLVM-C.lib`/`LLVM-C.dll`, and keeps the native job on the reviewed portable/runtime/ABI subset rather than Linux-only aggregate gates.
  - `build.zig` now exposes `test-portable`, shared `test-runtime-basic`, a filtered `test-runtime-windows` subset, and `sa-cli`; native runtime steps reject mismatched host/target combinations so Linux cross builds cannot be reported as native evidence.
  - `tools/ci/windows_native_smoke.ps1` stages the installed compiler, static runtime, generated header, and std sources under a space/Unicode release-like path; it isolates HOME/TEMP/plugin state and checks version/help/check, native Hello execution, wasm32 magic, offline package install/resolve, and missing-package `SourceNotFound` failure.
  - Static contract gate passes `4/4`; YAML/actionlint pass; Linux `portable-host-typecheck` passes `11/11`. This host has no Windows runner, so the workflow and PowerShell smoke have not been executed natively and Windows L2 remains unclaimed.
- Repaired a pre-run Windows LLVM provisioning defect found by inspecting the pinned official installer: it contains `LLVM-C.lib`/`LLVM-C.dll` but only two `llvm-c` headers, while the compiler shim requires the complete C API and generated target configuration.
...
- Implemented the first macOS L0/L1 gate-definition batch in `f03fc02`; native execution remains pending:
  ...
  - Linux-side evidence passes: macOS contract `3/3`, workflow YAML/actionlint and shell parsing, isolated `portability-check` `30/30`, and `test-portable` `9/9` steps with `49/49` tests. The x86_64/aarch64 static-runtime cross builds each pass `4/4`; `llvm-readobj` identifies both archive members as Mach-O x86_64 or arm64 as expected. These results include `portable-host-typecheck` `11/11`, dual-architecture runtime/shim checks, and aarch64 macOS ABI layout/header compilation.
...
- Implemented the shared basic/Darwin runtime-contract gate definition; native macOS and Windows execution remains pending:
  - `test-runtime-basic` links its C contract to the production static runtime and a target-built `.so`/`.dylib`/`.dll` fixture; `test-runtime-darwin` adds Darwin raw metadata timestamps, waitpid-path process status, and dylib behavior. Both steps reject non-native host/target execution instead of skipping to green.
  - Linux native `test-runtime-basic` passes `6/6`; `portability-check` passes `40/40`; `test-portable` passes `9/9` steps and `49/49` tests; static macOS/Windows CI contracts pass `3/3` and `4/4`. The post-review Windows driver and portable-host checks pass `2/2` and `11/11`.
  - Linux cross-link evidence passes for x86_64 and aarch64 Mach-O production static runtimes, basic/Darwin contract executables, and dylib fixtures, and for the x86_64 Windows PE basic-contract executable plus DLL fixture. These artifacts were not executed on macOS or Windows.
  - The Windows native linker now adds `ws2_32` after the installed static runtime inputs, and the static contract locks the workflow path from installed `sa_std.lib` through staged `sa build-exe` to generated EXE execution. The workflow itself has not run on Windows.
  - The process case proves exact small stdout/stderr and exit code only. POSIX capture still waits before draining pipes and can deadlock above pipe capacity; `sa_std_process_exec_capture` reads only one 8192-byte chunk per stream, so arbitrary-size capture remains open.
  - The macOS workflow invokes both native runtime gates and the Windows workflow invokes the shared basic gate, but neither workflow has run on its native runner; no Darwin/Windows runtime or L2 success is claimed.
- Completed the Darwin socket platformization and native-gate-definition batch; macOS execution remains pending:
  - Socket option reads/writes now use initialized `socklen_t` values with `std.posix.system.getsockopt`/`setsockopt`, so Darwin resolves libc calls instead of emitting Linux raw syscalls. Errno handling is stable for interruption, invalid handles/arguments, access, memory, and unsupported options without entering Zig 0.14.1 high-level `setsockopt` unreachable paths.
  - IPv4 TTL, IPv6 unicast hop limit, IPv4/IPv6 multicast options, TCP keepalive idle/interval/count, `SO.TYPE`, and `SO.ACCEPTCONN` use target-platform constants and payloads. TTL selection follows the socket family; IPv4 accepts `1..255`, while IPv6 accepts through `255`.
  - Darwin pathname UDS parsing stops at the first NUL and populates `sockaddr_un.len`. Abstract UDS, PASSCRED, peer credentials, QUICKACK, DEFER_ACCEPT, epoll including close, pidfd including all three `*_command_ext_pidfd(create_pidfd=1)` entry points, and netx return deterministic `UNSUPPORTED`; fallible outputs are cleared before returning. The C header now fixes the Unix-address free result type and exposes the network-family/Unix-address-kind constants locked by ABI tests.
  - `runtime-darwin-socket-link` cross-links the C contract to the production runtime, while `test-runtime-darwin-socket` refuses non-native execution and is wired into both macOS runner jobs. The contract covers DNS, TCP, UDP, IPv6 hop limit, pathname UDS, live socket options, and the unsupported/output-clearing boundary.
  - Linux evidence passes: focused socket tests `7/7`, complete runtime unit tests `74/74`, `portable-runtime-typecheck` `19/19`, `portability-check` `42/42`, `test-portable` `9/9` steps with `49/49` tests, `sa-std-abi` `11/11`, and macOS CI contract `3/3`. The Darwin C contract compiles with `-Wall -Wextra -Werror` for x86_64 and aarch64; production Mach-O runtime/socket links pass `4/4` for each target and dynamically reference `_getsockopt` plus `_setsockopt`.
  - `sa-std-runtime` remains at the known environment-limited `13/14`: this Linux container rejects the existing IPv6 multicast join test with exit `23`. No macOS runner has executed the new contract, and IPv4/IPv6 multicast join/leave is not exercised by it; option/payload coverage there remains system-header audit plus cross-compile evidence, not native behavior evidence.
- Completed the Darwin terminal winsize/native-PTY gate-definition batch; macOS execution remains pending:
  - `sa_term_winsize` now uses the target POSIX `ioctl(TIOCGWINSZ)` ABI instead of Linux-only raw ioctl constants, so Darwin and Linux share the same runtime export while non-terminal descriptors still return `UNSUPPORTED` with cleared `SaTermWinsize` output. Raw mode now also clears `ECHONL`.
  - `tests/runtime_darwin_pty_contract.c` builds only for Darwin, links to the production static runtime, opens a real PTY, sets `TIOCSWINSZ`, wraps the slave fd as a runtime handle, checks terminal detection, raw flag clearing including `ECHONL`, `VMIN=1`/`VTIME=0`, winsize preservation before and during raw mode, full termios restore through both `sa_term_raw_leave` and `sa_std_close(session)`, duplicate leave invalid-handle behavior, and non-terminal winsize `UNSUPPORTED` output clearing.
  - `runtime-darwin-pty-link` cross-links the contract for x86_64/aarch64 macOS, while `test-runtime-darwin-pty` refuses non-native execution and is wired into both macOS runner jobs. The static macOS CI contract locks the workflow and build graph fragments.
  - Linux executable evidence passes: `sa-term-runtime` `2/2`, `test-portable` `9/9` steps with `49/49` tests, and `sa-std-abi` `11/11`. Linux/cross evidence passes: `portable-runtime-typecheck` `21/21`, `portability-check` `44/44`, macOS CI contract `3/3`, format and diff checks, warnings-as-errors Darwin C compiles for x86_64/aarch64, and production Mach-O PTY contract links `4/4` for each target.
  - `sa-std-runtime` remains at the known environment-limited `13/14`: this Linux container rejects the existing IPv6 multicast join test with exit `23`. No macOS runner has executed the PTY contract, so this is a native gate definition plus Linux/cross evidence, not macOS native runtime evidence.
- Next portability order: execute and diagnose the macOS and Windows native workflows, then continue native plugin/installer/archive/release and daemon smoke. Harden POSIX process capture as an independent remaining risk before claiming arbitrary-size capture.
