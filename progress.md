...
- **Implementation pending focused verification — P0.3 incremental function objects:** `src/cli.zig` now reads only manifest-v2-authorized per-function objects, verifies exact relative path plus ordinary-file/nonzero size/SHA-256, emits misses through synced sibling temp files, links before atomically committing the new manifest, and cleans stale objects/temp files only after commit. Cache publication failure falls back to the valid transient object for the current link and skips manifest mutation. Function object keys use a v2 length-prefixed namespace and include the parent build key, full lowering signatures/slot mappings, canonical const values, instruction operands and verifier state deltas, debug locations when enabled, and the module-wide anonymous-string context consumed by LLVM lowering. The CLI same-size corruption repair gate and fresh combined build are still running; no verified-completion claim is made yet.
...
- Verification rule: only the tests named above are evidence. The latest focused snapshot passes `lib-root-smoke` filters for same-output publication pairing, single-flight owner handoff, and OOM no-partial publication, plus a direct `zig test` `OPTION_ENV!` absent→present filter; focused formatting and `git diff --check` pass for `src/cli.zig`/`tests/cli_smoke.zig`. The earlier combined `/opt/zig/zig build -Doptimize=Debug -j1` passed before the latest single-flight/output-stage delta, so a fresh combined build and authorization/affected/Referee closures remain. Full `zig fmt --check src tests` still reports untouched pre-existing files; no repository-wide format claim is made. Incremental-object integrity, failed two-file publication repair, broader corruption, and cross-consumer reachability gates remain.
...
- Next portability order: execute and diagnose the macOS and Windows native workflows, then continue Darwin sockets and terminal work. Harden POSIX process capture as an independent remaining risk before claiming arbitrary-size capture.
