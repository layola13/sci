...
- **In verification — P0.6 store-event writer identity:** project cache store events now record a source-free Linux process id and `sa cache status/why` exposes it as `last_store_writer_pid`. Existing event markers without this field remain backward-compatible and report `null`; focused single-flight/cache smoke and Debug build verification are pending.
