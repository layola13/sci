...
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_SORTED_FOLD_U64`, `BINARY_HEAP_INTO_ITER_SORTED_TRY_FOLD_U64`, `BINARY_HEAP_INTO_ITER_SORTED_SUM_U64`, `BINARY_HEAP_INTO_ITER_SORTED_PRODUCT_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MIN_U64`, `BINARY_HEAP_INTO_ITER_SORTED_MAX_U64`, `BINARY_HEAP_INTO_ITER_SORTED_REDUCE_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_TRY_REDUCE_U64`.
- Semantics: aliases consume the existing materialized descending cursor greatest-first and reuse concrete `u64` iterator fold/aggregate helpers. Reverse fold aliases are intentionally not added because Rust `IntoIterSorted` is not a `DoubleEndedIterator`.
- Test: `tests/unit_framework/std_binary_heap_into_iter_sorted_fold_aggregate_macro_surface.sa` - 1 test (panic ID 10776) covering fold, try_fold success/failure, sum/product/min/max, reduce, empty aggregate identities, try_reduce success/failure, cursor remainders, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_fold_aggregate_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
...
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_SORTED_NTH_U64`, `BINARY_HEAP_INTO_ITER_SORTED_COUNT_U64`, and `BINARY_HEAP_INTO_ITER_SORTED_LAST_U64`.
- Semantics: aliases consume the existing materialized descending cursor greatest-first and reuse concrete `u64` iterator cursor/terminal helpers. Reverse cursor aliases are intentionally not added because Rust `IntoIterSorted` is not a `DoubleEndedIterator`.
- Test: `tests/unit_framework/std_binary_heap_into_iter_sorted_cursor_ops_macro_surface.sa` - 1 test (panic ID 10772) covering next+nth composition, last, count, nth miss, empty last/count, cursor exhaustion, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_sorted_cursor_ops_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
  - Full tests intentionally not run because prior full runs caused memory pressure; this batch only runs the newly added focused test.
- Panic IDs next free: 10773+.

## Completed: 2026-07-17 BinaryHeap IntoIter for_each aliases

- Continued Rust `BinaryHeap::IntoIter` macro-surface parity, referencing local Rust `library/alloc/src/collections/binary_heap/mod.rs` for the owned heap iterator over backing storage and `library/core/src/iter/traits/iterator.rs` for `for_each` / `try_for_each`.
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_FOR_EACH_U64` and `BINARY_HEAP_INTO_ITER_TRY_FOR_EACH_U64`.
- Semantics: aliases consume the explicit owned backing-Vec cursor in heap-array backing order. `try_for_each` short-circuits after the callback returns false and leaves the suffix after the failing item available.
- Test: `tests/unit_framework/std_binary_heap_into_iter_for_each_macro_surface.sa` - 1 test (panic ID 10771) covering for_each exhaustion, try_for_each success/failure, empty try_for_each, cursor remainders, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_for_each_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
  - Full tests intentionally not run because prior full runs caused memory pressure; this batch only runs the newly added focused test.
...
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_FIND_U64`, `BINARY_HEAP_INTO_ITER_TRY_FIND_U64`, `BINARY_HEAP_INTO_ITER_FIND_MAP_U64`, `BINARY_HEAP_INTO_ITER_POSITION_U64`, `BINARY_HEAP_INTO_ITER_RPOSITION_U64`, and `BINARY_HEAP_INTO_ITER_RFIND_U64`.
- Semantics: aliases consume the explicit owned backing-Vec cursor in heap-array backing order, or from the back for reverse variants. Outputs use explicit bool, `(has, value)`, `(has, index)`, or `(ok, has, value)` scalar channels.
- Test: `tests/unit_framework/std_binary_heap_into_iter_search_macro_surface.sa` - 1 test (panic ID 10769) covering find hit/miss, try_find success/failure, find_map, position, rposition, rfind, cursor remainders, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_search_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
  - Full tests intentionally not run because prior full runs caused memory pressure; this batch only runs the newly added focused test.
- Panic IDs next free: 10770+.

## Completed: 2026-07-17 BinaryHeap IntoIter advance/chunk aliases

- Continued Rust `BinaryHeap::IntoIter` macro-surface parity, referencing local Rust `library/alloc/src/collections/binary_heap/mod.rs` for the owned heap iterator over backing storage, `library/core/src/iter/traits/iterator.rs` for `next_chunk` / `advance_by`, and `library/core/src/iter/traits/double_ended.rs` for `advance_back_by`.
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_TRY_NEXT_CHUNK_U64`, `BINARY_HEAP_INTO_ITER_ADVANCE_BY`, and `BINARY_HEAP_INTO_ITER_ADVANCE_BACK_BY`.
- Semantics: aliases consume the explicit owned backing-Vec cursor in heap-array backing order, or from the back for `advance_back_by`. Advance helpers return explicit `(ok, remaining)` scalars; chunk helper writes filled `u64` values into a caller buffer and reports `(ok, filled)`.
- Test: `tests/unit_framework/std_binary_heap_into_iter_advance_chunk_macro_surface.sa` - 1 test (panic ID 10768) covering forward/back advance success, short advance exhaustion, complete chunk, short chunk, zero chunk, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_advance_chunk_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
  - Full tests intentionally not run because prior full runs caused memory pressure; this batch only runs the newly added focused test.
- Panic IDs next free: 10769+.

## Completed: 2026-07-17 BinaryHeap IntoIter cursor ops aliases

- Continued Rust `BinaryHeap::IntoIter` macro-surface parity, referencing local Rust `library/alloc/src/collections/binary_heap/mod.rs` for the owned heap iterator over backing storage, `library/core/src/iter/traits/iterator.rs` for `nth`, `count`, and `last`, and `library/core/src/iter/traits/double_ended.rs` for `nth_back`.
- `sa_std/binary_heap.sa`: added `BINARY_HEAP_INTO_ITER_NTH_U64`, `BINARY_HEAP_INTO_ITER_NTH_BACK_U64`, `BINARY_HEAP_INTO_ITER_COUNT_U64`, and `BINARY_HEAP_INTO_ITER_LAST_U64`.
- Semantics: aliases consume the explicit owned backing-Vec cursor in heap-array backing order, or from the back for `nth_back`, and reuse the concrete `u64` iterator cursor/terminal helpers. Outputs use scalar count values or explicit `(has, value)` status channels.
- Test: `tests/unit_framework/std_binary_heap_into_iter_cursor_ops_macro_surface.sa` - 1 test (panic ID 10767) covering next+nth+nth_back composition, last, count, nth miss, nth_back miss, cursor exhaustion, and backing Vec ownership.
- Validation status:
  - Focused only: `SA_STD_DIR=/home/vscode/projects/sci/sa_std ./zig-out/bin/sa test tests/unit_framework/std_binary_heap_into_iter_cursor_ops_macro_surface.sa --jobs 1 --trace-panic` -> `1 passed; 0 failed; 0 skipped`.
  - Full tests intentionally not run because prior full runs caused memory pressure; this batch only runs the newly added focused test.
