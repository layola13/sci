...
- **P0.6 boundary:** this is still not full cache explanation. Fine-grained candidate old-entry key input first-difference reporting beyond the coarse `key.digest` field, richer write-event history/owner telemetry beyond result/stage/writer pid, full redaction review, and cross-process/cross-platform evidence remain open. The current `expired` reason is a read-only status/why policy, not build lookup or eviction semantics; the current `evicted` reason is limited to `sa cache clean` removals; the current `lock_owner_failed` evidence is limited to deterministic project-cache lock-path failure; ordinary test-cache metrics are verified only for successful cold/warm `sa test --json` after the runner exits, affected metrics are verified only for successful cold/warm `--affected --json` test runs, list metrics are verified only for JSON cold/disabled/hit paths, plugin bypass evidence is Linux-focused, dynamic-dependency bypass evidence uses a test-only same-request-change hook, and store-event evidence covers only the latest coarse successful/failed result, pipeline stage, and Linux writer pid without event history or cross-process owner lifecycle details.
