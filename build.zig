...
pub fn build(b: *std.Build) void {
    ...
    const runtime_darwin_socket_contract_module = b.createModule(.{
        .target = target,
        .optimize = optimize,
        .link_libc = true,
    });
    runtime_darwin_socket_contract_module.addIncludePath(b.path("src/runtime"));
    runtime_darwin_socket_contract_module.addCSourceFile(.{
        .file = b.path("tests/runtime_darwin_socket_contract.c"),
        .flags = &.{ "-std=c11", "-Wall", "-Wextra", "-Werror" },
    });
    runtime_darwin_socket_contract_module.linkLibrary(sa_std_static);
    const runtime_darwin_socket_contract = b.addExecutable(.{
        .name = "runtime-darwin-socket-contract",
        .root_module = runtime_darwin_socket_contract_module,
    });
    const runtime_darwin_socket_link_step = b.step("runtime-darwin-socket-link", "Link the Darwin socket runtime contract");
    if (target.result.os.tag == .macos and runtime_darwin_arch_supported) {
        runtime_darwin_socket_link_step.dependOn(&runtime_darwin_socket_contract.step);
    } else {
        const fail = b.addFail("runtime-darwin-socket-link requires a macOS x86_64 or aarch64 target");
        runtime_darwin_socket_link_step.dependOn(&fail.step);
    }
    const run_runtime_darwin_socket_contract = b.addRunArtifact(runtime_darwin_socket_contract);
    run_runtime_darwin_socket_contract.setCwd(repo_root_lazy);
    run_runtime_darwin_socket_contract.expectStdOutEqual("runtime Darwin socket contract ok\n");
    const test_runtime_darwin_socket_step = b.step("test-runtime-darwin-socket", "Run the native Darwin socket runtime contract");
    const runtime_darwin_socket_is_native = runtime_target_is_native and
        b.graph.host.result.os.tag == .macos and
        target.result.os.tag == .macos and
        runtime_darwin_arch_supported;
    if (runtime_darwin_socket_is_native) {
        test_runtime_darwin_socket_step.dependOn(&run_runtime_darwin_socket_contract.step);
    } else {
        const fail = b.addFail("test-runtime-darwin-socket requires a native macOS x86_64 or aarch64 host and target");
    ...
    for ([_][]const u8{ "x86_64-macos", "aarch64-macos" }) |darwin_target| {
        ...
        const darwin_socket_contract_typecheck = b.addSystemCommand(&.{
            b.graph.zig_exe,
            "cc",
            "-target",
            darwin_target,
        ...
        darwin_socket_contract_typecheck.addFileArg(b.path("tests/runtime_darwin_socket_contract.c"));
        darwin_socket_contract_typecheck.addArg("-o");
