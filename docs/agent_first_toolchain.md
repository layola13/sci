...
所有的基础命令（`explain`, `fix`, `skills`）、全局的 JSON 结构化输出能力，都将直接实现在 `src/cli.zig` 和 `src/common/trap.zig` 中。外部插件系统的作用是**把自己的 ABI、skills 和运行时能力放在独立工程里交付给主线读取**，而不是把业务语义回写进主线。
...
在刚设计的插件 ABI 中，增加对 Agent-First 范式的支持：
