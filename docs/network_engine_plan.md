...
> - SPSC / MPSC 取舍：**per-core sharded SPSC** 为主，复用现有 `sa_std/sync/mpsc.saasm` 仅作慢路径。
...
SA 标准库（现有 sa_std/）
├── net.saasm              ← 保留不动（旧版同步 TCP API）
├── net.saasm-iface        ← 保留不动
├── net.saasm-layout       ← 保留不动
├── core/
│   └── mem.saasm          ← 保留不动；网络热路径禁止调用（标量循环，仅冷路径用）
├── sync/
│   ├── mpsc.saasm         ← 保留不动；仅作跨分片回收的慢路径
│   └── mpsc.saasm-layout  ← 保留不动
└── 【新增】
    ├── netx.saasm-iface   # @extern 契约：sa_netx_* 一族 FFI 声明
    ├── netx.saasm-layout  # Ticket_SIZE / Ticket_slot_id / NetxProto_* 偏移
    └── netx.saasm         # @import 两个上面的文件，作为 SA 业务层入口
...
| `sa_std/netx.saasm-iface` | ~30 | 新增 |
| `sa_std/netx.saasm-layout` | ~20 | 新增 |
| `sa_std/netx.saasm` | ~5 | 新增 |
...
> ⚠️ 现有 `sa_std/sync/mpsc.saasm` 实际上是 **MPSC**（tail 上的 `cmpxchg` 决定多生产者）。本引擎默认采用**每核一对独立的 SPSC** 来彻底消除 cmpxchg 抖动；MPSC 仅作跨核回收等慢路径。
...
**为何 SPSC 而非 MPSC**：sharded 模型中 reactor↔SA 是 1:1，没有任何并发写入者；CAS 即使无竞争也比纯 store/load 慢 5–10 ns，对 ping-pong 跑分不可接受。MPSC 仅在跨分片回收（罕见）时使用现有 `sa_std/sync/mpsc.saasm` 的实现。

### 3.2 SA-ASM 边界对接（计划新增 FFI 契约，不修改现有代码）

> 以下声明应当在后续工作中追加到 `sa_std/net.saasm-iface` 与 `sa_std/net.saasm-layout`，本期仅作计划登记，**不动现有 sa_std**。

新增 `sa_std/netx.saasm-iface`（与现有 `net.saasm-iface` 共存）：
```
...
| 里程碑 | 描述 | 预期目标 / 验收标准 |
| :--- | :--- | :--- |
| **M0: 编译器准备** | 确认 SA-ASM 现有 ISA（atomic、ptr_add、load/store as T）足够支撑 §3.1 的 Ticket 直读；登记 `sa_netx.*` iface/layout 文件骨架。 | SA-ASM 无需新增 SIMD/bitcast 指令；新 iface/layout 文件评审通过。 |
...
| 现有 `sa_std/core/mem.saasm` 的 `sa_mem_set` 逐字节循环被误用于热路径 | 中 | 性能崩盘 | 新引擎 §1.1 明确规定连接池清零由 Zig `@memset` 处理，禁止 SA-ASM 路径触达 `sa_mem_set`/`sa_mem_copy` |
...
| 慢路径 MPSC（跨分片回收） | `sa_std/sync/mpsc.saasm` | 现有实现可用，仅作非热路径 |
| Memcpy/memset | `sa_std/core/mem.saasm` 标量循环 | **禁止用于网络热路径**；仅冷路径或单元测试用 |
