...
- IMP-1 重定位基础：新增已测试的 symbol-id remap helper，可将 `Instruction` 中的 `reg`/`symbol`/`label`/`func` operand 从缓存片段的 `SymbolTable` ID 映射到消费者 `SymbolTable` ID；函数签名 ID slice remap、raw_text 名称碰撞策略和 def/const 合并仍待完整设计。
