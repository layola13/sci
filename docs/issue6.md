...
- IMP-1 expanded-import cache correctness：expanded import fragment 现在会跟踪本次片段展开是否跳过了调用方已 seen 的 import；这种上下文相关片段只用于当前展开，不再写入进程级 expanded cache，避免后续消费者复用缺少传递函数、宏或 layout 的片段。新增回归测试覆盖“caller 先导入依赖再导入 wrapper，随后另一个文件只导入 wrapper”场景。
