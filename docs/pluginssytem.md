...
SA-ASM 主体只保留核心的所有权编译器 (Referee) 和指令执行引擎，而所有的扩展功能（例如数据库访问、HTTP 网络、大模型转换 `llvm2sa` 等）都由外部插件工程以动态库形式提供。当前这些插件都在 `~/projects/sa_plugins/` 下以独立工程维护，各自拥有自己的 `build.zig` 和 `src/plugin.zig`。
...
```zig
// ~/projects/sa_plugins/sa_plugin_math/src/plugin.zig
