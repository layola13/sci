...
当前分支已经把 macOS/Windows 的原生 CI 定义拆成可审计的命名门禁，避免 Linux-only aggregate gate 被误当成跨平台证据。Unix-socket daemon 目前有一个独立的 `daemon-smoke` gate：Linux 主机可执行地启动真实 `sa daemon`，通过 `SA_DAEMON_SOCKET` 转发一次 `sa version`，再验证 shutdown 和 socket 清理；macOS workflow 已接入该 gate，等待 x86_64/arm64 GitHub runner 产生原生执行证据。macOS/Windows native smoke 还会把本地 release archive 和 checksum sidecar 暴露为 release URL，再分别通过 `install.sh` / `install.ps1` 执行 clean install roundtrip；随后 smoke 脚本会把同一 archive/checksum 目录通过 loopback HTTP 暴露，并再次执行 installer，以覆盖 remote-download-style 安装路径。成功后 workflow 会通过共享的 `tools/ci/validate_native_evidence.zig` 校验 native-smoke JSON 证据文件，再上传 artifact，字段包括 schema version、platform、arch、Zig/LLVM version、archive、installer transports、staged/file-installed/http-installed version、wasm magic、pass marker，以及 GitHub SHA/run id/run attempt，便于后续审计原生运行结果并绑定到具体提交。runtime gates 也会在全部通过后写出并校验独立的 native-runtime JSON artifact：macOS 记录 plugin、daemon、basic/PAL/NetX、Darwin/socket/PTY gates，Windows 记录 basic/PAL/NetX/Windows runtime subset，并同样绑定 schema version、Zig/LLVM version 与 GitHub SHA/run id/run attempt。GitHub release matrix 仍只启用 Linux archive rows，并由 source contract 锁定：macOS/Windows release artifacts 必须等对应 native workflow 记录 compiler、runtime、installer、archive smoke 成功后才能打开。`release-contract` 现在也依赖 `native-evidence-validator`，所以未来打开 release matrix 前不能绕过 native-smoke/native-runtime JSON 证据规则。Windows 仍不声明 Unix-socket daemon gate，因为 daemon transport 当前按设计只支持非 Windows 平台。
