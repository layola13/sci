...
- **类型转换**：SA 是强类型的。你不能直接把 `i64` 当作 `ptr` 使用，必须通过编译器允许的显式指令。
- **内存块寻址效率**：运行时的解释器使用基于散列查找的快速内存查询 (blockIndexAt 已被重构为利用二分法及树结构)。这使得即便在大量分配小内存块时，`sa run` 的查询仍然保持在高效。

## Pro Tip: 内存池优化
由于 `alloc` 涉及系统调用开销，SA 开发者通常会预分配一个大型缓冲区并手动管理其偏移，或者直接使用 `sa_std` 提供的分配封装。
