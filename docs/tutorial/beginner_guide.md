...
SA 没有复杂的工程结构，一切从单个 `.saasm` 文件开始。新建一个 `hello.saasm`：
...
2. **构建本地可执行文件 (Native)**：调用底层的 LLVM/zig cc 生成极速二进制
   ```bash
   saasm build-exe hello.saasm -o hello
   ./hello
   ```
3. **构建 WebAssembly (WASM)**：无缝跨平台
   ```bash
   saasm build-wasm hello.saasm -o hello.wasm
   ```
4. **构建目标文件 (Object)**：用于和 C/C++/Rust 混编
   ```bash
   saasm build-obj hello.saasm -o hello.o
   ```
...
```
*提示：将来可以使用 `saasm layout` 命令行工具来自动帮你生成这些 `#def` 偏移量。*
...
SA 不仅仅是一个语言核心，它还拥有一个强大的**热插拔插件系统** (Hot-Pluggable Plugin System)。
