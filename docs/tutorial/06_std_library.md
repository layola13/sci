...
- **`*.saasm-iface` (接口)**：包含 `@extern` 声明。它定义了你能调用哪些函数。
- **`*.saasm-layout` (布局)**：包含 `#def` 常量。它定义了内存偏移量和错误代码。
- **`*.saasm` (实现)**：包含具体的 SA 代码和 `@macro`。它提供了高级封装。

**最佳实践**：
```saasm
@import "sa_std/vec.saasm-layout" // 导入布局（常量）
@import "sa_std/vec.saasm"        // 导入实现（宏）
// 注意：vec.saasm 内部通常会自动 @import 对应的 iface
```

## 2. 动态数组 (`Vec`)
`sa_std/vec.saasm` 提供了类似 C++ `std::vector` 的功能。

```saasm
@import "sa_std/vec.saasm-layout"
...
## 2. 字符串处理 (`String`)
SA 的原生字符串是 `utf8` 常量，但动态修改字符串需要 `sa_std/string.saasm`。
...
## 练习
1. 使用 `Vec` 存储 10 个随机数，并使用 `sa_std/sort.saasm` 对其进行排序。
2. 尝试读取用户输入并将其反转输出。
