...
## 什么是 SA？
SA (safe asm, 安全汇编) 是一种带所有权验证的线性汇编语言。它的目标不是提供高级语法糖，而是提供一套扁平、显式、容易由机器和 LLM 检查的低层 IR。
...
4.  **极简与高性能工具链**：内核层拥有出色的编译器缓存架构、预分配的解释器缓存池（O(1) 热路径检索），并提供了如 `NUM_U64_CHECKED_ADD` 溢出检查机制等底层安全宏。
5.  **FFI 气闸舱**：裸指针和外部 ABI 操作集中在 `@ffi_wrapper` 内。
