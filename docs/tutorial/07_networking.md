...
- **安全性防护**：网络层已进行深度的安全性与健壮性防护修复：包括了对接收到的 WebSocket 帧容量的安全阶段验证，安全验证防止截断时的惊恐。`TicketQueue` 本身重构实现了独立的 Payload 所有权（避免 UAF 以及裸指针引用）。对于网络分配槽（`ConnectionSlot`）也已加强了内存分配的大小检测，以防 OOM 或意外覆盖。
