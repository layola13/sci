...
> 本文档归纳 talk.md 第 2955 行起 14 轮架构讨论，定义 SA（safe asm，安全汇编）从工程管理、防投毒、权限隔离到 CI/CD 流水线的完整闭环。
>
...
| `postinstall` 隐式执行 → 投毒温床 | `sa pkg fetch` 是图灵不完备的"哑下载器"，无生命周期钩子 |
...
`sa.mod` 是正常工程包 manifest，负责源码包依赖、项目权限和插件需求。它不是 `sap.json`：`sap.json` 只属于编译器/宿主 native 插件。
...
- 默认 deny-all；没有声明权限时，不允许文件、网络、环境或进程能力。
- `fs` 必须拆分 `read/write/create/delete/metadata` 和路径范围；禁止 `/`、`/**`、`..`。
- `net` 远程只允许 `https://`；本地只允许 `localhost`、`127.0.0.1` 或 `[::1]`。
- `env` 必须列出变量名或受限前缀；禁止 `*`。
- `process.spawn` 默认 `false`；允许时必须列出可执行文件和参数策略。
- `sa.mod` 权限用于透明源码包静态审计；`sap.json` 权限用于 native 插件安装和 sandbox enforcement。

`sa.mod` 与 `sap.json` 的最大共同点：都声明 identity、dependencies、sha256 integrity、permissions，并默认零权限。

最大区别：`sa.mod` 管正常工程包和透明源码依赖，类似 Rust crate；`sap.json` 管编译器/宿主插件和 native artifact，必须有 descriptor、symbol smoke 和安装级 sandbox。
...
`sa install` 读取当前目录的 `sa.mod`，执行两条 resolver：
...
可以学习 Deno 的交互体验，但语义必须更严格：`--allow-*` 是**本次命令的用户授权上限**，不是依赖包或插件的自我声明。

有效权限必须同时满足：

```text
effective_permission = manifest_declared_permission ∩ cli_allow_permission
```

也就是说：

- `sa.mod.permissions` / `sap.json.permissions` 没声明的能力，即使用户传了 `--allow-*` 也不能用。
- manifest 声明了能力，但用户本次没有 `--allow-*`，也不能用。
- 安装 privileged 插件仍必须人工确认；`--allow-*` 不能替代插件安装确认。

推荐 CLI：

```sh
sa run app.sa --allow-env=HOME,SA_*
sa run app.sa --allow-read=$PROJECT/data/** --allow-write=$PROJECT/out/**
sa run app.sa --allow-net=https://api.example.com,http://localhost:8787
sa run app.sa --allow-run=/usr/bin/git
sa build app.sa --allow-net=https://api.example.com
sa install --allow-net=https://github.com --allow-read=$PROJECT/** --allow-write=$PROJECT/sa_vendor/**
```

规则：

- `--allow-env=NAME,PREFIX_*`：只允许列出的环境变量或前缀。
- `--allow-read=PATH,...` / `--allow-write=PATH,...`：路径必须 canonicalize，拒绝 `..`、根目录通配和符号链接逃逸。
- `--allow-net=URL,...`：远程只允许 `https://...`；本地只允许 localhost/loopback。
- `--allow-run=ABS_PATH,...`：必须是绝对路径，不能是裸命令名。
- 裸 `--allow-net` / `--allow-read` 不应表示全开放；最多表示“允许 manifest 已声明的对应权限”。真正全开放只允许 dev 模式，不能进 CI 默认路径。
- 不提供 `-A` / `--allow-all` 作为正式安全入口。需要开发便利时使用显式 `--dev` 或 `SA_DEV=1`，并在输出中标记非生产模式。

### 2.5 `permission_sets` — 项目级权限预设

Deno 2.5+ 支持在 `deno.json/deno.jsonc` 中保存命名权限集，并通过 `-P=<name>` / `--permission-set=<name>` 显式启用。SA 可以采用同类体验，但放在 `sa.mod`，因为 `sa.mod` 已经是工程 manifest。
...
permission_set dev {
  env [HOME, SA_*]
  read [$PROJECT/**]
  write [$PROJECT/.sa-cache/**, $PROJECT/out/**]
  net [https://api.example.com, http://localhost:8787]
  run [/usr/bin/git]
}
...
permission_set test {
  env [CI, HOME]
  read [$PROJECT/tests/**, $PROJECT/sa_vendor/**]
  write [$PROJECT/.test-out/**]
  net []
  run []
}
```
...
```sh
sa run app.sa -P=dev
sa test tests/main.sa -P=test
sa build app.sa --permission-set=dev
```

安全规则：

- `permission_set` 不会自动生效，必须显式 `-P=<name>`。
- `permission_set default` 也不能静默启用；最多允许 `-P` 省略名字时选择 default。
- CLI 临时 `--allow-*` 可以进一步收窄或补充本次授权，但不能超过 `sa.mod.permissions` / `sap.json.permissions` 声明。
- CI 中如果 `sa.mod` 声明了权限集但命令没有 `-P` 或 `--allow-*`，应失败并提示选择权限集，避免误以为权限已经生效。

### 2.6 大型依赖树的确认策略

上千依赖不能逐个手工确认。`sa install` 必须先构建完整安装计划，再按风险分层确认。
...
- **自动放行**：零权限源码包、hash 已在 `sa.lock/sa.sum` 中且源码未变的依赖。
- **自动拒绝**：hash 不匹配、权限超出 `permission_set`/`--allow-*`、远程 `http://`、裸二进制插件、无 `sap.json` 插件。
- **外部 URL 必须确认**：任何新增外部 URL 都是强确认点，包括 `require` 源码包 URL、`require_plugin` 插件来源 URL、`sap.json.source.url`、`permissions.net[].url`。黑客可能通过新增依赖或改 URL 偷偷上传数据，所以新增外部 URL 不能自动放行。
- **批量确认**：首次引入一批源码包时，只展示“新增权限差异”和整棵依赖图 hash，让用户确认一次，而不是每个包一次。
- **高风险单独确认**：native 插件、`process.spawn`、写权限、`https://*` 这类广域网络权限必须单独列出；可以在同一个 review 页面中确认，但不能被隐藏在普通依赖里。
- **锁定后免确认**：确认结果写入 `sa.lock` / `sap.lock` / `permissions.lock`。之后只要 dependency graph hash、源码 hash、artifact hash、权限账本都不变，不再弹确认。
- **变更才确认**：新增依赖、新增权限、hash 改变、插件 ABI 改变、权限集改变时才重新确认。
- **CI 不交互**：CI 只接受已提交 lock/policy。需要人工确认的计划在 CI 中直接失败，并输出 review plan。

外部 URL 确认规则：

- 第一次出现的外部 URL 必须人工确认，并写入 `permissions.lock`。
- URL 的 host、scheme、port、path 前缀任一变化，都视为新增 URL。
- `https://api.example.com/v1` 与 `https://api.example.com/admin` 是不同权限边界，不能自动合并。
- 远程源码包和插件源码必须固定 commit/tag + `sha256`；只确认 host 不足以放行代码。
- 组织级 `sa.policy` 可以预先 allow 某些 URL，但必须由项目维护者提交，不能由依赖包生成。
- `--dev` 只能跳过本地路径确认；不能在正式模式下跳过新增外部 URL 确认。

推荐命令：

```sh
sa install --review
sa install --permission-set=dev
sa install --write-lock
sa install --ci
```

`--review` 只生成计划，不安装；`--write-lock` 在人工确认后写锁；`--ci` 不允许任何 prompt。

企业/团队模式可以增加 `sa.policy`：
...
`sa.policy` 是组织级 allow/deny 规则，用来减少重复确认；但它必须显式提交和审计，不能由依赖包自己生成。
...
```
1. 项目级 ./sa_vendor/<URL>/
2. 全局缓存 ~/.sa/pkg/<URL>/        （仅当 sa pkg fetch -g 拉过）
...
| npm 的死穴 | SA 的物理熔断 |
|---|---|
| `npm install` 自动执行 `postinstall` | `sa pkg fetch` 是绝对图灵不完备的哑巴操作 |
...
## 7. `sa pkg fetch` —— 出厂自带 X 光扫描

### 7.1 物理运转：拉取即审计

```bash
sa pkg fetch github.com/org/image-parser
```
...
>> Run `sa pkg audit github.com/hacker/bad-lib` locally to clear this warning.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
```
...
/金融场景常要求开发者本地生成 Linux/Windows/macOS 全平台机器码哈希并签名。但同一份源码在三个平台编译出的机器码不同，开发者手里通常只有一台 Mac 或 Windows。
...
### 13.1 痛点
 ...
 / 金融 / 企业的 CI/CD 服务器是**物理拔网线**的。SA 编译器如果硬编码连 GitHub 验 SHA，根本进不了大门。

### 13.2 解法 1：全源码入库（Vendor Committal）

```bash
# 在桥接机（有外网）
sa pkg fetch                       # 拉到 ./sa_vendor/
...
sa pkg fetch [URL]                    # 拉到局部 ./sa_vendor/
sa pkg fetch -g [URL]                 # 拉到全局 ~/.sa/pkg/
sa pkg fetch --offline                # 仅校验现有 sa_vendor/，不联网

sa pkg audit [URL]                    # 重新跑 X 光扫描 + 生成报告
sa pkg audit --update-lock            # 把当前机器码 hash 写入 sa.lock
```
...
| 命令 | 失败时是否修改文件 |
|---|---|
| `sa pkg fetch` | 仅落盘源码到 `sa_vendor/`，**绝不**修改 `sa.mod` / `sa.lock` |
| `sa build` 触发审判台 | **绝不**写盘，仅内存豁免 |
| `sa pkg audit --update-lock` | **唯一**允许写 `sa.lock` 的命令；显式动作 |
...
- **审计日志归档**：长期保留 `sa pkg fetch` 时的 X 光报告（默认仅打印到 stdout，不归档）。
...
```
项目根/
├── sa.mod              # 极简依赖清单（手写 + sa pkg fetch 增量补全）
├── sa.lock             # 项目级哈希锁（sa pkg audit --update-lock 写入）
...
└── src/
    └── main.sa
~/.sa/                  # 仅 sa pkg fetch -g 时使用，不存豁免状态
└── pkg/
    └── github.com/...
```
