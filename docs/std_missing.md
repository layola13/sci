...
*   **Implemented in `sa_std`**: `PATH_MAKE_EMPTY`, `PATH_BASENAME`, `PATH_DIRNAME`, `PATH_STEM`, `PATH_EXT`, `PATH_IS_ABSOLUTE`, `PATH_HAS_ROOT`, `PATH_STARTS_WITH`, `PATH_TRY_STRIP_PREFIX`, `PATH_JOIN`, `PATH_PARENT`, `PATH_FILE_NAME`, `PATH_FILE_STEM`, `PATH_EXTENSION`, `PATH_STRIP_EXTENSION`, `PATH_WITH_FILE_NAME`, `PATH_WITH_EXTENSION`, `PATH_COMPONENT_COUNT`, `PATH_TRY_COMPONENT_AT`, `PATH_TRY_COMPONENT_PREFIX_AT`.
...
*   **Scope note**: The SA path facade is a POSIX-style byte-slice subset over `Slice`. `STARTS_WITH` and `TRY_STRIP_PREFIX` enforce a simple slash boundary, `JOIN`, `WITH_FILE_NAME`, and `WITH_EXTENSION` materialize concatenated string slices through existing `STR_CONCAT`, and `PARENT` / `FILE_NAME` / `FILE_STEM` / `EXTENSION` are aliases over concrete basename/dirname/stem/ext helpers. `PATH_COMPONENT_COUNT` counts non-empty slash-separated POSIX components, `PATH_TRY_COMPONENT_AT` returns a caller-indexed component `Slice`, and `PATH_TRY_COMPONENT_PREFIX_AT` returns the path prefix through that component. These are queryable component/ancestor views, not Rust lazy iterator adapters. The module does not model Rust's platform-specific `OsStr`, owned `PathBuf`, Windows prefixes, filesystem queries, or borrow-scoped path types.
