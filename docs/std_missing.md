...
*   **Implemented in `sa_std`**: `PATH_MAKE_EMPTY`, `PATH_BASENAME`, `PATH_DIRNAME`, `PATH_STEM`, `PATH_EXT`, `PATH_IS_ABSOLUTE`, `PATH_HAS_ROOT`, `PATH_STARTS_WITH`, `PATH_TRY_STRIP_PREFIX`, `PATH_JOIN`, `PATH_PARENT`, `PATH_FILE_NAME`, `PATH_FILE_STEM`, `PATH_EXTENSION`, `PATH_STRIP_EXTENSION`, `PATH_WITH_FILE_NAME`, `PATH_WITH_EXTENSION`, `PATH_COMPONENT_COUNT`.
*   **Missing from Rust**: Type-safe `Path` / `PathBuf` system, component iterator APIs (`components`, `iter`, `ancestors`), filesystem-backed predicates (`exists`, `is_file`, `is_dir`), `canonicalize`, symlink handling, Windows prefix/root semantics, and owned `PathBuf` capacity/mutation behavior.
*   **Scope note**: The SA path facade is a POSIX-style byte-slice subset over `Slice`. `STARTS_WITH` and `TRY_STRIP_PREFIX` enforce a simple slash boundary, `JOIN`, `WITH_FILE_NAME`, and `WITH_EXTENSION` materialize concatenated string slices through existing `STR_CONCAT`, and `PARENT` / `FILE_NAME` / `FILE_STEM` / `EXTENSION` are aliases over concrete basename/dirname/stem/ext helpers. `PATH_COMPONENT_COUNT` counts non-empty slash-separated POSIX components, not Rust's platform-specific component iterator. It does not model Rust's platform-specific `OsStr`, `PathBuf`, Windows prefixes, or component iterator semantics.
