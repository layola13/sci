...
*   **Missing Methods**: `str` methods like `chars`, iterator-based `bytes`, iterator-based `split`, iterator-based `lines`, Unicode whitespace `trim`, full Rust `Pattern`-based `replace`, and UTF-8 boundary helpers. The `STRING_BUF_*` surface models Rust `String`'s owned `Vec<u8>` capacity and mutation subset, but it does not yet implement full UTF-8 validation, iterator adapters, or Rust's full owned `String` method set.
*   **Rust-aligned byte-view helpers**: `STR_BYTES` / `STRING_BYTES`, `STR_TRY_FIND` / `STRING_TRY_FIND`, `STR_TRY_RFIND` / `STRING_TRY_RFIND`, `STR_TRY_FIND_BYTE` / `STRING_TRY_FIND_BYTE`, `STR_TRY_RFIND_BYTE` / `STRING_TRY_RFIND_BYTE`, `STR_COUNT_BYTE` / `STRING_COUNT_BYTE`, `STR_SPLIT_BYTE_COUNT` / `STRING_SPLIT_BYTE_COUNT`, `STR_TRY_SPLIT_BYTE_AT` / `STRING_TRY_SPLIT_BYTE_AT`, `STR_LINE_COUNT` / `STRING_LINE_COUNT`, `STR_TRY_LINE_AT` / `STRING_TRY_LINE_AT`, `STR_TRY_SPLIT_ONCE` / `STRING_TRY_SPLIT_ONCE`, and `STR_TRY_RSPLIT_ONCE` / `STRING_TRY_RSPLIT_ONCE` provide byte-slice views and byte search/count subsets, returning `(found, ...)` rather than Rust `Option<...>` where needed.
