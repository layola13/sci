...
*   **Implemented in `sa_std`**: `SLICE_NEW`, `SLICE_GET_PTR`, `SLICE_AS_PTR`, `SLICE_GET_LEN`, `SLICE_IS_EMPTY`, `SLICE_GET_U64`, `SLICE_GET_MUT_PTR_U64`, `SLICE_FIRST_U64`, `SLICE_FIRST_MUT_PTR_U64`, `SLICE_LAST_U64`, `SLICE_LAST_MUT_PTR_U64`, `SLICE_TRY_FIRST_U64`, `SLICE_TRY_LAST_U64`, `SLICE_TRY_GET_U64`, `SLICE_TRY_GET_MUT_PTR_U64`, `SLICE_TRY_FIRST_MUT_PTR_U64`, `SLICE_TRY_LAST_MUT_PTR_U64`, `SLICE_TRY_SPLIT_FIRST_U64`, `SLICE_TRY_SPLIT_LAST_U64`, `SLICE_CONTAINS_U64`, `SLICE_STARTS_WITH_U64`, `SLICE_ENDS_WITH_U64`, `SLICE_TRY_STRIP_PREFIX_U64`, `SLICE_TRY_STRIP_SUFFIX_U64`, `SLICE_TRIM_PREFIX_U64`, `SLICE_TRIM_SUFFIX_U64`, `SLICE_TRY_SPLIT_AT_U64`, `SLICE_TRY_SPLIT_AT_MUT_U64`, `SLICE_TRY_RANGE_U64`, `SLICE_TRY_GET_RANGE_U64`, `SLICE_CHUNK_COUNT_U64`, `SLICE_TRY_CHUNK_AT_U64`, `SLICE_WINDOW_COUNT_U64`, `SLICE_TRY_WINDOW_AT_U64`, `SLICE_SWAP_U64`, `SLICE_TRY_SWAP_U64`, `SLICE_FILL_U64`, `SLICE_REVERSE_U64`, `SLICE_COPY_FROM_SLICE_U64`, `SLICE_CLONE_FROM_SLICE_U64`, `SLICE_COPY_WITHIN_U64`, `SLICE_TRY_BINARY_SEARCH_U64`, `SLICE_PARTITION_POINT_U64`, `SLICE_LOWER_BOUND_U64`, `SLICE_UPPER_BOUND_U64`, `SLICE_EQUAL_RANGE_U64`.
*   **Missing from Rust**: generic element support, scoped Rust-reference `first_mut` / `last_mut` semantics, generic range-based `get`, true split/chunk/window iterator adapters, unchecked split variants, sorting, and iterator APIs.
*   **Scope note**: Current slice comparison, mutation, copy, range-view, split-view, chunk-view, window-view, and search helpers are concrete `u64` helpers. `SLICE_TRY_SPLIT_FIRST_U64` / `SLICE_TRY_SPLIT_LAST_U64` return concrete `(ok, value, rest_slice)` views rather than Rust borrow-scoped option/reference tuples. Chunk/window helpers return caller-indexed `Slice` views, not Rust lazy iterator adapters. Binary-search helpers assume ascending `u64` slices; `SLICE_PARTITION_POINT_U64` uses a caller-supplied `u64 -> u64` predicate, while `LOWER_BOUND`, `UPPER_BOUND`, and `EQUAL_RANGE` provide concrete index pairs instead of Rust generic comparator APIs. Mutable helpers return raw element pointers for verifier-friendly in-place updates; they intentionally do not pretend to be Rust's generic `[T]` trait surface or borrow-scoped reference semantics.
...
*   **Implemented in `sa_std`**: `BINARY_HEAP_NEW`, `BINARY_HEAP_WITH_CAPACITY`, `BINARY_HEAP_TRY_WITH_CAPACITY`, `BINARY_HEAP_FREE`, `BINARY_HEAP_LEN`, `BINARY_HEAP_CAPACITY`, `BINARY_HEAP_RESERVE`, `BINARY_HEAP_RESERVE_EXACT`, `BINARY_HEAP_TRY_RESERVE`, `BINARY_HEAP_TRY_RESERVE_EXACT`, `BINARY_HEAP_SHRINK_TO`, `BINARY_HEAP_SHRINK_TO_FIT`, `BINARY_HEAP_AS_SLICE`, `BINARY_HEAP_AS_MUT_SLICE`, `BINARY_HEAP_INTO_VEC`, `BINARY_HEAP_INTO_SORTED_VEC`, `BINARY_HEAP_IS_EMPTY`, `BINARY_HEAP_PEEK`, `BINARY_HEAP_TRY_PEEK_MUT_PTR`, `BINARY_HEAP_PEEK_MUT_PTR`, `BINARY_HEAP_PUSH`, `BINARY_HEAP_TRY_POP`, `BINARY_HEAP_CLEAR`, `BINARY_HEAP_RETAIN_U64`, `BINARY_HEAP_APPEND`.
*   **Missing from Rust**: `peek_mut`, `into_iter`, `iter`, `drain`, generic ordering support, allocator-aware constructors, and true allocation-failure reporting for `try_reserve*`.
*   **Scope note**: The SA facade is a concrete `u64` max-heap. `AS_SLICE` / `AS_MUT_SLICE` expose the internal heap array order, `INTO_VEC` reinterprets the owned heap storage as a `Vec`-layout object, and `INTO_SORTED_VEC` consumes the heap and returns ascending `u64` values. `BINARY_HEAP_TRY_PEEK_MUT_PTR` returns a raw pointer to the root slot, so callers can update the top element in place without Rust's scoped borrow semantics. `BINARY_HEAP_RETAIN_U64` uses a concrete `u64 -> u64` predicate, compacts retained values in place, and rebuilds heap order before returning. It does not implement Rust's generic ordering, iterator, mutable peek, or allocator error semantics.
...
*   **Implemented in `sa_std`**: `PATH_MAKE_EMPTY`, `PATH_BASENAME`, `PATH_DIRNAME`, `PATH_STEM`, `PATH_EXT`, `PATH_IS_ABSOLUTE`, `PATH_HAS_ROOT`, `PATH_IS_RELATIVE`, `PATH_IS_EMPTY`, `PATH_STARTS_WITH`, `PATH_TRY_STRIP_PREFIX`, `PATH_ENDS_WITH`, `PATH_JOIN`, `PATH_PARENT`, `PATH_FILE_NAME`, `PATH_FILE_PREFIX`, `PATH_FILE_STEM`, `PATH_EXTENSION`, `PATH_STRIP_EXTENSION`, `PATH_WITH_FILE_NAME`, `PATH_WITH_EXTENSION`, `PATH_COMPONENT_COUNT`, `PATH_TRY_COMPONENT_AT`, `PATH_TRY_COMPONENT_PREFIX_AT`, `PATH_ANCESTOR_COUNT`, `PATH_TRY_ANCESTOR_AT`.
*   **Missing from Rust**: Type-safe `Path` / `PathBuf` system, true component iterator APIs (`components`, `iter`, `ancestors`), filesystem-backed predicates (`exists`, `is_file`, `is_dir`), `canonicalize`, symlink handling, Windows prefix/root semantics, and owned `PathBuf` capacity/mutation behavior.
*   **Scope note**: The SA path facade is a POSIX-style byte-slice subset over `Slice`. `STARTS_WITH`, `ENDS_WITH`, and `TRY_STRIP_PREFIX` enforce slash-boundary component semantics, `JOIN`, `WITH_FILE_NAME`, and `WITH_EXTENSION` materialize concatenated string slices through existing `STR_CONCAT`, and `PARENT` / `FILE_NAME` / `FILE_STEM` / `EXTENSION` are aliases over concrete basename/dirname/stem/ext helpers. `PATH_FILE_PREFIX` mirrors Rust's first-dot split including hidden filename handling, `PATH_COMPONENT_COUNT` counts non-empty slash-separated POSIX components, `PATH_TRY_COMPONENT_AT` returns a caller-indexed component `Slice`, `PATH_TRY_COMPONENT_PREFIX_AT` returns the path prefix through that component, and `PATH_ANCESTOR_COUNT` / `PATH_TRY_ANCESTOR_AT` expose indexed ancestor views. These are queryable component/ancestor views, not Rust lazy iterator adapters. The module does not model Rust's platform-specific `OsStr`, owned `PathBuf`, Windows prefixes, filesystem queries, or borrow-scoped path types.

### 1.14 Time & Sync (`std::time`, `std::sync` vs `sa_std/time.sa`, `sa_std/sync/*`)
*   **Implemented in `sa_std`**: `Instant` / `Unix` timestamps, `Sleep`, `Duration` nanosecond construction and conversion helpers, `TIME_DURATION_ZERO`, `TIME_DURATION_MAX`, `TIME_DURATION_IS_ZERO`, `TIME_DURATION_ABS_DIFF_NS`, `duration_since` / `checked_duration_since` / `elapsed` macro subsets, checked add/sub/mul/div helpers with Rust-style `TIME_DURATION_CHECKED_*` aliases, saturating add/sub/mul/div helpers with Rust-style `TIME_DURATION_SATURATING_*` aliases, `subsec_nanos` / `subsec_micros` / `subsec_millis`, MPSC channels, `Mutex` (spin), `Once`, `RwLock`, `Arc`, `RefCell` shared/exclusive borrow helpers, and the matching core macros in `sa_std/core/*`.
*   **Plugin alternative, not `sa_std`**: Deno facade time helpers live in `sa_plugin_deno` and are not part of compiler std coverage.
*   **Missing from Rust (Time)**: Rust's typed `Duration` / `Instant` / `SystemTime` structs, signed/float duration conversions, `SystemTimeError`, platform-specific clock semantics, and rigorous overflow/error semantics beyond the current `(ok, value)` nanosecond macro subset. `TIME_SATURATING_DIV_NS` is an SA convenience over checked division and saturates division-by-zero to `u64::MAX`; Rust `Duration` has `checked_div` and arithmetic trait division but no exact same saturating-div method.
