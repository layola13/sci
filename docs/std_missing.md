...
*   **Scope note**: Current slice comparison, mutation, copy, range-view, split-view, chunk-view, window-view, and search helpers are concrete `u64` helpers. Chunk/window helpers return caller-indexed `Slice` views, not Rust lazy iterator adapters. Mutable helpers return raw element pointers for verifier-friendly in-place updates; they intentionally do not pretend to be Rust's generic `[T]` trait surface or borrow-scoped reference semantics.
