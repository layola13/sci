...
由于 SA 采用“零信任沙箱”架构，HTTP 客户端被设计为可热插拔的独立 Linux 动态库 (`.so`)，且所有出站请求均受到 `sa.pkg` 包管理器的严格审计与拦截。
...
- **零拷贝流水线 (Zero-Copy)**: 为了极致性能，响应体数据尽可能直接透传；在 `sa_http_server` 的反向代理场景下，请求可以直接转发出站，绕过用户态内存拷贝。
