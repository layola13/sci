...
2026-07-17 补充检查点：store-event 现在还会写入 source-free per-key history 记录，并在 `sa cache status/why` 暴露 `last_store_event_count`。focused single-flight 证据覆盖失败 `copy_output` 后计数为 `1`，以及后续 waiter 成功发布后最新事件为 `published/publish` 且历史计数为 `2`。同一 focused gate 还验证失败和成功两种 `sa cache why --json` 输出只暴露 12 字符 `key_prefix`，不包含完整 64 字符 cache key。这只收窄“事件历史”和 key 脱敏缺口；process start identity、跨进程 owner 生命周期、完整脱敏审计和跨平台证据仍未完成。
