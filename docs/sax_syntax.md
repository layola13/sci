...
ffi_wrapper_def ::= "@ffi_wrapper" IDENT ":" NEWLINE
                    sa_block
...
### 3.5 `@ffi_wrapper:` 气闸舱函数

`@ffi_wrapper` 是 SAX 内的安全边界。SAX Lowerer 会把涉及 DOM、Airlock、路由初始化、计时器和原生逃逸的实现体放进 `@ffi_wrapper`，然后再由薄的 `@export` 包装导出给外部调用。

```xml
@ffi_wrapper mount_dom(ctx: ptr):
L_ENTRY:
  view = assume_borrow ctx
  raw = *view
  call @sax_dom_query(*SAX_App_host_app, 4)
  !raw
  !view
  return
```
...
规则：
- `@ffi_wrapper` 内允许使用 `$...$` 原生逃逸
- `@ffi_wrapper` 外禁止出现 `$...$`
- `@ffi_wrapper` 仍然是普通 SA 函数体，必须满足标准控制流和 `ret` 终止要求
- SAX 生成的组件入口会以 `@export` 对外暴露，内部实现由 `@ffi_wrapper` 承载
