...
sa_control_flow ::= sa_label | sa_jmp | sa_br   // 直通，与 .saasm 一致
...
组件内部的事件处理逻辑，语法与 `.saasm` 函数体完全一致（SA-ASM 风格）。
...
<Component name="TodoList">
  ...
  <state>
    todos     = alloc 24    // []TodoItem 胖指针（data_ptr+len+cap）
...
<Component name="DataTable">
  ...
  L_BODY:
    row_ptr = call @sax_array_get(&data, i)
    call @sax_table_append_row(&tbody_node, row_ptr)
    i = add i, 1
    jmp L_LOOP
  L_END:
    ret
...
SAX 是 `.saasm` 的超集，`@handler` 函数体内的所有语法**与 `.saasm` 完全一致**：
